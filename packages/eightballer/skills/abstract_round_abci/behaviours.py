...
from packages.eightballer.skills.abstract_round_abci.models import SharedState
...
class _MetaRoundBehaviour(ABCMeta):
    ...
    def __new__(cls, name: str, bases: tuple, namespace: dict, **kwargs: Any) -> type:  # type: ignore
        ...
        cls.are_background_behaviours_set = bool(new_cls.background_behaviours_cls - {PendingOffencesBehaviour})
        cls._check_consistency(cast(AbstractRoundBehaviour, new_cls))
        return new_cls

    @classmethod
    def _check_consistency(cls, behaviour_cls: "AbstractRoundBehaviour") -> None:
        """Check consistency of class attributes."""
        cls._check_all_required_classattributes_are_set(behaviour_cls)
        cls._check_behaviour_id_uniqueness(behaviour_cls)
        cls._check_initial_behaviour_in_set_of_behaviours(behaviour_cls)
        cls._check_matching_round_consistency(behaviour_cls)

    @classmethod
    def _check_all_required_classattributes_are_set(cls, behaviour_cls: "AbstractRoundBehaviour") -> None:
    ...
    @classmethod
    def _check_behaviour_id_uniqueness(cls, behaviour_cls: "AbstractRoundBehaviour") -> None:
        """Check that behaviour ids are unique across behaviours."""
        behaviour_id_to_behaviour = defaultdict(list)
        for behaviour_class in behaviour_cls.behaviours:
            behaviour_id_to_behaviour[behaviour_class.auto_behaviour_id()].append(behaviour_class)
    ...
    def _check_matching_round_consistency(cls, behaviour_cls: "AbstractRoundBehaviour") -> None:
        ...
        matching_bg_round_classes = {
            behaviour_cls.matching_round for behaviour_cls in behaviour_cls.background_behaviours_cls
        }
        round_to_behaviour: dict[type[AbstractRound], list[BehaviourType]] = {
            ...
            for round_cls in behaviour_cls.abci_app_cls.get_all_round_classes(
                matching_bg_round_classes,
                cls.are_background_behaviours_set,
            )
        }
...
class PendingOffencesBehaviour(BaseBehaviour):
    ...
    def async_act(self) -> Generator:
        ...
        payload = PendingOffencesPayload(self.context.agent_address, *asdict(offence).values())
...
class AbstractRoundBehaviour(  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._behaviour_id_to_behaviours: dict[str, BehaviourType] = self._get_behaviour_id_to_behaviour_mapping(
            self.behaviours
        )
        self._round_to_behaviour: dict[type[AbstractRound], BehaviourType] = self._get_round_to_behaviour_mapping(
            self.behaviours
        )

        self.current_behaviour: BaseBehaviour | None = None
        self.background_behaviours: set[BaseBehaviour] = set()
        self.tm_manager: TmManager | None = None
        ...
        self._next_behaviour_cls: BehaviourType | None = None

    @classmethod
    def _get_behaviour_id_to_behaviour_mapping(cls, behaviours: AbstractSet[BehaviourType]) -> dict[str, BehaviourType]:
    ...
    def instantiate_behaviour_cls(self, behaviour_cls: BehaviourType) -> BaseBehaviour:
        ...
        return behaviour_cls(name=behaviour_cls.auto_behaviour_id(), skill_context=self.context)
    ...
    def _setup_background(self) -> None:
        ...
        for background_cls in self.background_behaviours_cls:
            ...
            self.background_behaviours.add(self.instantiate_behaviour_cls(background_cls))

    def setup(self) -> None:
        """Set up the behaviours."""
        self.current_behaviour = self.instantiate_behaviour_cls(self.initial_behaviour_cls)
    ...
    def _process_current_round(self) -> None:
        ...
        if self.current_behaviour is not None and self._last_round_height == current_round_height:
