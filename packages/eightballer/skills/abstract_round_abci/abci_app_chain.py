...
_default_logger = logging.getLogger("aea.packages.eightballer.skills.abstract_round_abci.abci_app_chain")

AbciAppTransitionMapping = dict[AppState, AppState]


def check_set_uniqueness(sets: tuple) -> Any | None:
...
def chain(  # pylint: disable=too-many-locals,too-many-statements
    ...
    non_abstract_abci_apps = [abci_app.__name__ for abci_app in abci_apps if not abci_app.is_abstract()]
    enforce(
    ...
    round_ids = tuple({round_.auto_round_id() for round_ in app.get_all_rounds()} for app in abci_apps)
    ...
    all_initial_states = {initial_state for app in abci_apps for initial_state in app.initial_states}.union(
        {app.initial_round_cls for app in abci_apps}
    )
    for key, value in abci_app_transition_mapping.items():
        if key not in all_final_states:
            msg = f"Found non-final state {key} specified in abci_app_transition_mapping."
            raise ValueError(msg)
        if value not in all_initial_states:
            msg = f"Found non-initial state {value} specified in abci_app_transition_mapping."
            raise ValueError(msg)
    ...
    def get_paths(
        ...
        app: type[AbciApp],
        previous_apps: list[type[AbciApp]] | None = None,
    ) -> list[list[tuple[AppState, type[AbciApp], AppState | None]]]:
        """Get paths."""
        previous_apps_: list[type[AbciApp]] = deepcopy(previous_apps) if previous_apps is not None else []
        default: list[list[tuple[AppState, type[AbciApp], AppState | None]]] = [[(initial_state, app, None)]]
        if app.final_states == {}:
            return default  # pragma: no cover
        paths: list[list[tuple[AppState, type[AbciApp], AppState | None]]] = []
        for final_state in app.final_states:
            element: tuple[AppState, type[AbciApp], AppState | None] = (
                initial_state,
                app,
                final_state,
            )
    ...
    all_paths: list[list[tuple[AppState, type[AbciApp], AppState | None]]] = get_paths(
        abci_apps[0].initial_round_cls, abci_apps[0]
    )
    new_db_post_conditions: dict[AppState, set[str]] = {}
    for path in all_paths:
        current_initial_state, current_app, current_final_state = path[0]
        accumulated_post_conditions: set[str] = current_app.db_pre_conditions.get(current_initial_state, set())
    ...
    new_final_states = {state for state in potential_final_states if state in destination_states}
    ...
    class ComposedAbciApp(AbciApp[EventType]):
        ...
        cross_period_persisted_keys: frozenset[str] = frozenset(new_cross_period_persisted_keys)
