...
from dataclasses import field, asdict, astuple, dataclass, is_dataclass
...
_logger = logging.getLogger("aea.packages.eightballer.skills.abstract_round_abci.base")
...
class Transaction(ABC):
    ...
    def encode(self) -> bytes:
        ...
        if sys.getsizeof(encoded_data) > MAX_READ_IN_BYTES:
            raise ValueError(f"Transaction must be smaller than {MAX_READ_IN_BYTES} bytes")
        return encoded_data
    ...
    def verify(self, ledger_id: str) -> None:
        ...
        addresses = LedgerApis.recover_message(identifier=ledger_id, message=payload_bytes, signature=self.signature)
...
class Blockchain:
    ...
    def add_block(self, block: Block) -> None:
        ...
        if expected_height != actual_height:
            raise AddBlockError(f"expected height {expected_height}, got {actual_height}")
...
class AbciAppDB:
    ...
    * the in-built `copy` module is used, which automatically detects if an item is immutable and skips copying it.
    ...
    def _cross_period_check(self) -> None:
        ...
        not_in_cross_period = set(self._setup_data).difference(self.cross_period_persisted_keys)
    ...
    def get_latest_from_reset_index(self, reset_index: int) -> Dict[str, Any]:
        ...
        return {key: values[-1] for key, values in deepcopy(self._data.get(reset_index, {})).items()}
    ...
    def cleanup(
        ...
        self._data = {key: self._data[key] for key in sorted(self._data.keys())[-cleanup_history_depth:]}
    ...
    def cleanup_current_histories(self, cleanup_history_depth_current: int) -> None:
        ...
        cleanup_history_depth_current = max(cleanup_history_depth_current, MIN_HISTORY_DEPTH)
        self._data[self.reset_index] = {
            key: history[-cleanup_history_depth_current:] for key, history in self._data[self.reset_index].items()
        }
    ...
    def sync(self, serialized_data: str) -> None:
        ...
        try:
            ...
            raise ABCIAppInternalError(f"Could not decode data using {serialized_data}: {exc}") from exc
...
class BaseSynchronizedData:
    ...
    def update(
        ...
        **kwargs: Any,
        ...
        class_ = type(self) if synchronized_data_class is None else synchronized_data_class
...
class _MetaAbstractRound(ABCMeta):
    ...
    @classmethod
    def _check_required_class_attributes(mcs, abstract_round_cls: Type["AbstractRound"]) -> None:
        """Check that required class attributes are set."""
        required_class_attributes = getattr(abstract_round_cls, "required_class_attributes", None)
        if not required_class_attributes:
            raise TypeError(f"Class {abstract_round_cls.__name__} must define `required_class_attributes`!")

        for attr in required_class_attributes:
            if not hasattr(abstract_round_cls, attr):
                raise AbstractRoundInternalError(f"'{attr}' not set in {abstract_round_cls}!")
...
class AbstractRound(Generic[EventType], ABC, metaclass=_MetaAbstractRound):
    ...
    def auto_round_id(cls) -> str:
        ...
        """
        return cls.round_id if isinstance(cls.round_id, str) else cls.__pattern.sub("_", cls.__name__).lower()
    ...
    def check_payload_type(self, transaction: Transaction) -> None:
        ...
        if self.payload_class is None:
            raise TransactionTypeNotRecognizedError("current round does not allow transactions")

        payload_class = type(transaction.payload)

        if payload_class is self._previous_round_payload_class:
            raise LateArrivingTransaction(f"request '{transaction.payload}' is from previous round; skipping")
    ...
    def check_majority_possible_with_new_voter(
        ...
        self.check_majority_possible(votes_by_participant, nb_participants, exception_cls=exception_cls)
    ...
    def check_majority_possible(
        ...
        if nb_remaining_votes + largest_nb_votes < self.synchronized_data.consensus_threshold:
            raise exception_cls(
    ...
    def is_majority_possible(self, votes_by_participant: Dict[str, BaseTxPayload], nb_participants: int) -> bool:
        """
...
class CollectionRound(AbstractRound, ABC):
    ...
    def deserialize_collection(
        ...
        """Deserialize a serialized collection."""
        return {address: BaseTxPayload.from_json(payload_json) for address, payload_json in serialized.items()}
    ...
    def process_payload(self, payload: BaseTxPayload) -> None:
        ...
        if sender not in self.accepting_payloads_from:
            raise ABCIAppInternalError(f"{sender} not in list of participants: {sorted(self.accepting_payloads_from)}")

        if sender in self.collection:
            raise ABCIAppInternalError(f"sender {sender} has already sent value for round: {self.round_id}")
    ...
    def check_payload(self, payload: BaseTxPayload) -> None:
        ...
        if payload.sender in self.collection:
            raise TransactionNotValidError(f"sender {payload.sender} has already sent value for round: {self.round_id}")
...
class CollectSameUntilAllRound(_CollectUntilAllRound, ABC):
    ...
    def check_payload(self, payload: BaseTxPayload) -> None:
        ...
        if payload.sender not in self.collection and len(self.collection) and new not in existing_:
            raise TransactionNotValidError(
    ...
    def common_payload_values(
        ...
        """Get the common payload among the agents."""
        most_common_payload_values, max_votes = self.payload_values_count.most_common(1)[0]
...
class CollectSameUntilThresholdRound(CollectionRound, ABC):
    ...
    def threshold_reached(
        ...
        return any(count >= self.synchronized_data.consensus_threshold for count in counts)
    ...
    def most_voted_payload_values(
        ...
        most_voted_payload_values, max_votes = self.payload_values_count.most_common()[0]
    ...
    def end_block(self) -> Optional[Tuple[BaseSynchronizedData, Enum]]:
        """Process the end of the block."""
        if self.threshold_reached and any([val is not None for val in self.most_voted_payload_values]):
        ...
        if self.threshold_reached and not any([val is not None for val in self.most_voted_payload_values]):
            return self.synchronized_data, self.none_event
        if not self.is_majority_possible(self.collection, self.synchronized_data.nb_participants):
...
class OnlyKeeperSendsRound(AbstractRound, ABC):
    ...
    def check_payload(self, payload: BaseTxPayload) -> None:
        ...
        sender_is_elected_sender = sender == self.synchronized_data.most_voted_keeper_address
    ...
    def end_block(self) -> Optional[Tuple[BaseSynchronizedData, Enum]]:
        """Process the end of the block."""
        if self.keeper_payload is not None and any([val is not None for val in self.keeper_payload.values]):
            ...
            return synchronized_data, self.done_event
        if self.keeper_payload is not None and not any([val is not None for val in self.keeper_payload.values]):
...
class CollectDifferentUntilThresholdRound(CollectionRound, ABC):
    ...
    def end_block(self) -> Optional[Tuple[BaseSynchronizedData, Enum]]:
        ...
        if self.collection_threshold_reached and self.block_confirmations > self.required_block_confirmations:
            synchronized_data = self.synchronized_data.update(
                synchronized_data_class=self.synchronized_data_class,
                **{
                    self.collection_key: self.serialized_collection,
...
class CollectNonEmptyUntilThresholdRound(CollectDifferentUntilThresholdRound, ABC):
    ...
    def _get_non_empty_values(self) -> Dict[str, Tuple[Any, ...]]:
        ...
        for sender, payload in self.collection.items():
            if sender not in non_empty_values:
                non_empty_values[sender] = [value for value in payload.values if value is not None]
                if len(non_empty_values[sender]) == 0:
                    del non_empty_values[sender]
                continue
        non_empty_values_ = {sender: tuple(li) for sender, li in non_empty_values.items()}
    ...
    def end_block(self) -> Optional[Tuple[BaseSynchronizedData, Enum]]:
        ...
        if self.collection_threshold_reached and self.block_confirmations > self.required_block_confirmations:
            ...
            if isinstance(self.selection_key, tuple):
                data: Dict[str, Any] = {
                    sender: dict(zip(self.selection_key, values)) for sender, values in non_empty_values.items()
                }
            else:
                data = {
                    self.selection_key: {sender: values[0] for sender, values in non_empty_values.items()},
                }
            data[self.collection_key] = self.serialized_collection
...
class _MetaAbciApp(ABCMeta):
    ...
    def _check_initial_states_and_final_states(
        ...
        enforce(
            ...
            f"initial round class {initial_round_cls} is not in the set of " f"initial states: {initial_states}",
        )
        enforce(
            initial_round_cls in states and all(initial_state in states for initial_state in initial_states),
            "initial states must be in the set of states",
        )

        true_initial_states = initial_states if initial_states != set() else {initial_round_cls}
        enforce(
            all(initial_state not in final_states for initial_state in true_initial_states),
        ...
        enforce(
            ...
            f"the following final states are not in the set of states:" f" {unknown_final_states}",
        )

        enforce(
            all(len(transition_function[final_state]) == 0 for final_state in final_states),
            "final states cannot have outgoing transitions",
        )

        enforce(
            all(issubclass(final_state, DegenerateRound) for final_state in final_states),
    ...
    def _check_db_constraints_consistency(mcs, abci_app_cls: Type["AbciApp"]) -> None:
        ...
        invalid_initial_states = set.difference(expected, actual) if is_pre_conditions_set else set()
        enforce(
        ...
        invalid_final_states = set.difference(expected, actual) if is_post_conditions_set else set()
        enforce(
            len(invalid_final_states) == 0,
            f"db post conditions contain invalid final states: {invalid_final_states}",
        )
        all_pre_conditions = {value for values in abci_app_cls.db_pre_conditions.values() for value in values}
        all_post_conditions = {value for values in abci_app_cls.db_post_conditions.values() for value in values}
        enforce(
            len(all_pre_conditions.intersection(all_post_conditions)) == 0,
            "db pre and post conditions intersect",
        )
        intersection = abci_app_cls.default_db_preconditions.intersection(all_pre_conditions)
        enforce(
            len(intersection) == 0,
            f"db pre conditions contain value that is a default pre condition: {intersection}",
        )
        intersection = abci_app_cls.default_db_preconditions.intersection(all_post_conditions)
        enforce(
            len(intersection) == 0,
    ...
    @classmethod
    def _check_consistency_outgoing_transitions_from_non_final_states(mcs, abci_app_cls: Type["AbciApp"]) -> None:
        """
        ...
        for non_final_state in non_final_states:
            ...
            enforce(
                ...
                f"non-final state {non_final_state} must have at least one " f"non-timeout transition",
            )
...
class BackgroundApp(Generic[EventType]):
    ...
    def __init__(
        ...
        _logger.debug(f"Created background app of type '{self.type}' using {given_args}.")
    ...
    def specify_type(self) -> BackgroundAppType:
        ...
        if self.start_event is None and self.end_event is None and self.transition_function is None:
            self.transition_function = {}
            return BackgroundAppType.EVER_RUNNING
        if self.start_event is not None and self.end_event is None and self.transition_function is not None:
            return BackgroundAppType.TERMINATING
        if self.start_event is not None and self.end_event is not None and self.transition_function is not None:
            return BackgroundAppType.NORMAL
        return BackgroundAppType.INCORRECT  # pragma: nocover

    def setup(self, initial_synchronized_data: BaseSynchronizedData, context: SkillContext) -> None:
    ...
    def process_transaction(self, transaction: Transaction, dry: bool = False) -> bool:
        ...
        if payload_class is bg_payload_class:
            processor = self.background_round.check_transaction if dry else self.background_round.process_transaction
...
class AbciApp(Generic[EventType], ABC, metaclass=_MetaAbciApp):  # pylint: disable=too-many-instance-attributes
    """
    ...
    def add_background_app(
        ...
        cross_period_keys = config.abci_app.cross_period_persisted_keys if config.abci_app is not None else frozenset()
        cls.cross_period_persisted_keys = cls.cross_period_persisted_keys.union(cross_period_keys)
        return cls
    ...
    def synchronized_data(self) -> BaseSynchronizedData:
        ...
        result = (
            ...
            if isclass(synchronized_data_class) and issubclass(synchronized_data_class, BaseSynchronizedData)
            else latest_result
        )
        return result
    ...
    def get_all_round_classes(
        ...
        if include_background_rounds:
            for app in cls.background_apps:
                if app.type != BackgroundAppType.EVER_RUNNING and app.round_cls in bg_round_cls:
    ...
    def setup(self) -> None:
        ...
        sync_classes = {_round.synchronized_data_class for _round in self.transition_function}
        ...
        update = {db_key: self._get_synced_value(db_key, sync_classes) for db_key in self.cross_period_persisted_keys}
        self.synchronized_data.db.update(**update)

    def _log_start(self) -> None:
        """Log the entering in the round."""
        self.logger.info(
            f"Entered in the '{self.current_round.round_id}' round for period " f"{self.synchronized_data.period_count}"
        )

    def _log_end(self, event: EventType) -> None:
        """Log the exiting from the round."""
        self.logger.info(f"'{self.current_round.round_id}' round is done with event: {event}")
    ...
    def schedule_round(self, round_cls: AppState) -> None:
        ...
        self._current_round = round_cls(
            ...
            (
                ...
                and self._last_round.payload_class != self._current_round_cls.payload_class
    ...
    def process_transaction(self, transaction: Transaction, dry: bool = False) -> None:
        ...
        processor = self.current_round.check_transaction if dry else self.current_round.process_transaction
        processor(transaction)

    def _resolve_bg_transition(self, app: BackgroundApp, event: EventType) -> Tuple[bool, Optional[AppState]]:
        """
        ...
        if app.type in (BackgroundAppType.NORMAL, BackgroundAppType.TERMINATING) and event == app.start_event:
            app.transition_function = cast(AbciAppTransitionFunction, app.transition_function)
            ...
            self._transition_backup.transition_function = deepcopy(self.transition_function)
            self.transition_function = app.transition_function
            self.logger.info(
                f"The {event} event was produced, transitioning to " f"`{next_round_cls.auto_round_id()}`."
            )
    ...
    def _adjust_transition_fn(self, event: EventType) -> None:
        ...
        for app in self.background_apps:
            if app.type == BackgroundAppType.NORMAL and event == app.end_event:
                ...
                self._switched = True
                self.logger.info(f"The {app.end_event} event was produced. Switching back to the normal FSM.")
    ...
    def process_event(self, event: EventType, result: Optional[BaseSynchronizedData] = None) -> None:
        """Process a round event."""
        if self._current_round_cls is None:
            self.logger.warning(f"Cannot process event '{event}' as current state is not set")
            return
    ...
    def update_time(self, timestamp: datetime.datetime) -> None:
        ...
        while earliest_deadline <= timestamp:
            ...
            # the impact)
            self._last_timestamp = timestamp
            self.logger.warning("current AbciApp time after expired deadline: %s", self.last_timestamp)
    ...
    def cleanup(
        ...
        self.synchronized_data.db.cleanup(cleanup_history_depth, cleanup_history_depth_current)

    def cleanup_current_histories(self, cleanup_history_depth_current: int) -> None:
        """Reset the parameter histories for the current entry (period), keeping only the latest values for each parameter."""
        self.synchronized_data.db.cleanup_current_histories(cleanup_history_depth_current)
...
class AvailabilityWindow:
    ...
    def __init__(self, max_length: int) -> None:
        ...
        """
        if max_length < 1:
            raise ValueError(f"An `AvailabilityWindow` with a `max_length` {max_length} < 1 is not valid.")
    ...
    def to_dict(self) -> Dict[str, int]:
        ...
        return {
            ...
            "array": (int("".join(str(int(flag)) for flag in self._window), base=2) if len(self._window) else 0),
            "num_positive": self._num_positive,
            "num_negative": self._num_negative,
        }

    @staticmethod
    def _validate_key(data: Dict[str, int], key: str, validator: Callable[[int], bool]) -> None:
...
class OffenceStatus:
    ...
    validator_downtime: AvailabilityWindow = field(default_factory=lambda: AvailabilityWindow(NUMBER_OF_BLOCKS_TRACKED))
    invalid_payload: AvailabilityWindow = field(default_factory=lambda: AvailabilityWindow(NUMBER_OF_ROUNDS_TRACKED))
    blacklisted: AvailabilityWindow = field(default_factory=lambda: AvailabilityWindow(NUMBER_OF_ROUNDS_TRACKED))
    suspected: AvailabilityWindow = field(default_factory=lambda: AvailabilityWindow(NUMBER_OF_ROUNDS_TRACKED))
...
class OffenseStatusDecoder(json.JSONDecoder):
    ...
    @staticmethod
    def hook(data: Dict[str, Any]) -> Union[AvailabilityWindow, OffenceStatus, Dict[str, OffenceStatus]]:
...
class RoundSequence:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, context: SkillContext, abci_app_cls: Type[AbciApp]):
        ...
        self._block_construction_phase = RoundSequence._BlockConstructionState.WAITING_FOR_BEGIN_BLOCK
    ...
    def check_is_finished(self) -> None:
        """Check if a round sequence has finished."""
        if self.is_finished:
            raise ValueError("round sequence is finished, cannot accept new transactions")
    ...
    def last_timestamp(self) -> datetime.datetime:
        ...
        last_timestamp = self._blockchain.blocks[-1].timestamp if self._blockchain.length != 0 else None
    ...
    def last_round_transition_tm_height(self) -> int:
        ...
        if self._last_round_transition_tm_height is None:
            raise ValueError("Trying to access Tendermint's last round transition height before any `end_block` calls.")
    ...
    def set_block_stall_deadline(self) -> None:
        ...
        self._block_stall_deadline = datetime.datetime.now() + datetime.timedelta(seconds=BLOCKS_STALL_TOLERANCE)
    ...
    def _track_tm_offences(self, evidences: Evidences, last_commit_info: LastCommitInfo) -> None:
        ...
        for byzantine_validator in evidences.byzantine_validators:
            ...
            self.offence_status[agent_address].num_unknown_offenses += bool(evidence_type == EvidenceType.UNKNOWN)
            self.offence_status[agent_address].num_double_signed += bool(evidence_type == EvidenceType.DUPLICATE_VOTE)
    ...
    def _try_track_offences(self, evidences: Evidences, last_commit_info: LastCommitInfo) -> None:
    ...
    def begin_block(
        ...
        """Begin block."""
        if self.is_finished:
            raise ABCIAppInternalError("round sequence is finished, cannot accept new blocks")
        if self._block_construction_phase != RoundSequence._BlockConstructionState.WAITING_FOR_BEGIN_BLOCK:
            raise ABCIAppInternalError(
        ...
        self._block_construction_phase = RoundSequence._BlockConstructionState.WAITING_FOR_DELIVER_TX
    ...
    def deliver_tx(self, transaction: Transaction) -> None:
        ...
        """
        if self._block_construction_phase != RoundSequence._BlockConstructionState.WAITING_FOR_DELIVER_TX:
            raise ABCIAppInternalError(
    ...
    def end_block(self) -> None:
        ...
        self._block_construction_phase = RoundSequence._BlockConstructionState.WAITING_FOR_COMMIT

    def commit(self) -> None:
        """Process the 'commit' request."""
        if self._block_construction_phase != RoundSequence._BlockConstructionState.WAITING_FOR_COMMIT:
            raise ABCIAppInternalError(
    ...
    def _get_round_result(
        ...
        for prioritized_group in self.abci_app.bg_apps_prioritized:
            for app in prioritized_group:
                ...
                if result is None or app.type == BackgroundAppType.TERMINATING and self._terminating_round_called:
                    continue
                if app.type == BackgroundAppType.TERMINATING and not self._terminating_round_called:
    ...
    def reset_state(
        ...
        round_id_to_cls = {cls.auto_round_id(): cls for cls in self.abci_app.transition_function}
...
class PendingOffencesRound(CollectSameUntilThresholdRound):
    ...
    def end_block(self) -> None:
        ...
        if offence.offense_type == OffenseType.CUSTOM:
            self.offence_status[offence.accused_agent_address].custom_offences_amount += offence.custom_amount
