...
from packages.eightballer.skills.abstract_round_abci import handlers
...
from packages.eightballer.skills.abstract_round_abci.test_tools.rounds import DummyRound


def test_exception_to_info_msg() -> None:
    """Test 'exception_to_info_msg' helper function."""
    exception = Exception("exception message")
    expected_string = f"{exception.__class__.__name__}: {exception!s}"
...
class TestABCIRoundHandler:
    ...
    def setup(self) -> None:
        ...
        self.context.state.round_sequence.last_round_transition_timestamp = datetime.now()
    ...
    def test_info(self) -> None:
        ...
        response = self.handler.info(cast(AbciMessage, message), cast(AbciDialogue, dialogue))
    ...
    def test_init_chain(self, app_hash: bytes) -> None:
        ...
        response = self.handler.init_chain(cast(AbciMessage, message), cast(AbciDialogue, dialogue))
    ...
    def test_begin_block(self) -> None:
        ...
        response = self.handler.begin_block(cast(AbciMessage, message), cast(AbciDialogue, dialogue))
    ...
    def test_check_tx(self, *_: Any) -> None:
        ...
        response = self.handler.check_tx(cast(AbciMessage, message), cast(AbciDialogue, dialogue))
    ...
    def test_deliver_tx(self, *_: Any) -> None:
        ...
        with mock.patch.object(self.context.state.round_sequence, "add_pending_offence") as mock_add_pending_offence:
            response = self.handler.deliver_tx(cast(AbciMessage, message), cast(AbciDialogue, dialogue))
    ...
    def test_deliver_bad_tx(self, *_: Any) -> None:
        ...
        with (
            ...
            mock.patch.object(self.context.state.round_sequence, "add_pending_offence") as mock_add_pending_offence,
        ):
            response = self.handler.deliver_tx(cast(AbciMessage, message), cast(AbciDialogue, dialogue))
            mock_add_pending_offence.assert_called_once()
    ...
    def test_commit(self) -> None:
        ...
        response = self.handler.commit(cast(AbciMessage, message), cast(AbciDialogue, dialogue))
    ...
    def test_commit_negative(self) -> None:
        ...
        with pytest.raises(AddBlockError):
            self.handler.commit(cast(AbciMessage, message), cast(AbciDialogue, dialogue))
...
class TestAbstractResponseHandler:
    ...
    def test_handle(self) -> None:
        ...
        with mock.patch.object(self.handler, "_recover_protocol_dialogues") as mock_dialogues_fn:
    ...
    @mock.patch.object(AbstractResponseHandler, "_recover_protocol_dialogues", return_value=None)
    ...
    def test_handle_negative_cannot_update_dialogues(self, mock_dialogues_fn: Any) -> None:
    ...
    def test_handle_negative_cannot_find_callback(self) -> None:
        ...
        self.context.requests.request_id_to_callback = {}
        with pytest.raises(ABCIAppInternalError, match="No callback defined for request with nonce: "):
...
class TestTendermintHandler:
    ...
    def test_handle_not_in_registered_addresses(self, caplog: LogCaptureFixture) -> None:
    ...
    def test_recovery_params(
        ...
        if performative == TendermintMessage.Performative.GET_RECOVERY_PARAMS:
            self.context.state.tm_recovery_params = TendermintRecoveryParams("DummyRound")
            message = TendermintMessage(performative)  # type: ignore
            log_message = self.handler.LogMessages.sending_request_response.value
        elif performative == TendermintMessage.Performative.RECOVERY_PARAMS:
            params = json.dumps(asdict(TendermintRecoveryParams(DummyRound.auto_round_id())))
            ...
            msg = f"Invalid performative {performative} for `test_recovery_params`."
    ...
    def test_sender_not_in_registered_addresses(self, caplog: LogCaptureFixture) -> None:
