...
import packages.eightballer.skills.abstract_round_abci.base as abci_base
...
from packages.eightballer.skills.abstract_round_abci.test_tools.rounds import (
    BaseRoundTestClass,
    get_participants,
)
from packages.eightballer.skills.abstract_round_abci.test_tools.abci_app import (
    AbciAppTest,
    ConcreteEvents,
    ConcreteRoundA,
    ConcreteRoundB,
...
def test_base_tx_payload() -> None:
    ...
    assert payload_data.pop("id_") != new_payload_data.pop("id_")
...
def test_specific_round_instantiation_without_extended_requirements_raises_error() -> None:
...
@mock.patch("aea.crypto.ledger_apis.LedgerApis.recover_message", return_value={"wrong_sender"})
def test_verify_transaction_negative_case(*_mocks: Any) -> None:
    ...
    with pytest.raises(SignatureNotValidError, match="Signature not valid on transaction: .*"):
        transaction.verify("")
...
class TestBlockBuilder:
    ...
    def test_transitions_getter(self) -> None:
        ...
        assert self.block_builder.transactions == ()
...
class TestAbciAppDB:
    ...
    def setup(self) -> None:
        ...
        self.db = AbciAppDB(
            setup_data={"participants": [self.participants]},
        )
    ...
    def test_init(
        ...
        cross_period_persisted_keys: set[str] | None,
        ...
        cast_keys = cast(frozenset[str] | None, cross_period_persisted_keys)
        ...
        if setup_data is None:
            ...
            with pytest.raises(
                ValueError,
                match=re.escape(f"AbciAppDB data must be `Dict[str, List[Any]]`, found `{type(data)}` instead"),
            ):
                AbciAppDB(
                    data,
                )
    ...
    def test_get(self) -> None:
        ...
        assert self.db.get_latest_from_reset_index(0) == {"participants": self.participants}
    ...
    @pytest.mark.parametrize(
        ...
        [
            ({}, {"dummy_key": "dummy_value"}, {0: {"dummy_key": ["dummy_value"]}}),
            (
                {},
    ...
    def test_update(self, setup_data: dict, update_data: dict, expected_data: dict[int, dict]) -> None:
        """Test update db."""
    ...
    def test_serialize(self) -> None:
        ...
        assert self.db.serialize() == '{"db_data": {"0": {"participants": [["a", "b"]]}}, "slashing_config": ""}'
    ...
    def test_sync(self, _data: dict[str, dict[int, dict[str, list[Any]]]]) -> None:
        ...
        try:
            ...
            msg = "Incorrectly parametrized test. Data must be json serializable."
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                json.dumps({"db_data": {"invalid_index": {}}, "slashing_config": "anything"}),
    ...
    def test_hash(self) -> None:
        """Test `hash` method."""
        expected_hash = (
            b"\xd0^\xb0\x85\xf1\xf5\xd2\xe8\xe8\x85\xda\x1a\x99k" b"\x1c\xde\xfa1\x8a\x87\xcc\xd7q?\xdf\xbbofz\xfb\x7fI"
        )
...
class TestBaseSynchronizedData:
    ...
    def setup(self) -> None:
        ...
        self.base_synchronized_data = BaseSynchronizedData(
            db=AbciAppDB(setup_data={"participants": [self.participants]})
        )
    ...
    def test_participants_getter_negative(self) -> None:
        ...
        with pytest.raises(
            ValueError,
            match=re.escape("'participants' field is not set for this period [0] and no default value was provided."),
        ):
    ...
    def test_update(self) -> None:
        ...
        participants = ("a",)
        expected = BaseSynchronizedData(db=AbciAppDB(setup_data={"participants": [participants]}))
    ...
    def test_create(self) -> None:
        ...
        self.base_synchronized_data.db._cross_period_persisted_keys = frozenset({"participants"})
        assert self.base_synchronized_data.db._data == {0: {"participants": [("a", "b")]}}
    ...
    def test_participants_list_is_empty(
        ...
        base_synchronized_data = BaseSynchronizedData(db=AbciAppDB(setup_data={"participants": [()]}))
    ...
    def test_all_participants_list_is_empty(
        ...
        """Tets when participants list is set to zero."""
        base_synchronized_data = BaseSynchronizedData(db=AbciAppDB(setup_data={"all_participants": [()]}))
        with pytest.raises(ValueError, match="List participants cannot be empty."):
    ...
    def test_consensus_threshold(self, n_participants: int, given_threshold: int, expected_threshold: int) -> None:
    ...
    def test_properties(self) -> None:
        ...
        participants = ["b", "a"]
        randomness_str = "3439d92d58e47d342131d446a3abe264396dd264717897af30525c98408c834f"
                ...
                setup_data=AbciAppDB.data_to_lists(
                    {
                        ...
                        "participant_to_votes": CollectionRound.serialize_collection(participant_to_votes),
        ...
        assert base_synchronized_data.most_voted_keeper_address == most_voted_keeper_address
        assert base_synchronized_data.is_keeper_set
        assert base_synchronized_data.blacklisted_keepers == {blacklisted_keepers}
        assert base_synchronized_data.participant_to_selection == participant_to_selection
        assert base_synchronized_data.participant_to_randomness == participant_to_randomness
...
class DummyConcreteRound(AbstractRound):
    ...
    payload_class: type[BaseTxPayload] | None = None
    synchronized_data_class = MagicMock()
...
class TestAbstractRound:
    ...
    def test_must_set_payload_class_type(self) -> None:
        ...
        with pytest.raises(AbstractRoundInternalError, match="'payload_class' not set in .*"):
    ...
    def test_process_transaction_negative_check_transaction_fails(self) -> None:
        ...
        with mock.patch.object(self.round, "check_payload_type", side_effect=ValueError(error_message)):
    ...
    def test_check_majority_possible_raises_error_when_nb_participants_is_0(
        ...
        with pytest.raises(
            ...
            DummyConcreteRound(self.base_synchronized_data, MagicMock()).check_majority_possible({}, 0)
    ...
    def test_check_majority_possible_passes_when_vote_set_is_empty(self) -> None:
        ...
        DummyConcreteRound(self.base_synchronized_data, MagicMock()).check_majority_possible({}, 1)
    ...
    def test_check_majority_possible_passes_when_vote_set_nonempty_and_check_passes(
        ...
        """
        DummyConcreteRound(self.base_synchronized_data, MagicMock()).check_majority_possible(
    ...
    def test_is_majority_possible_positive_case(self) -> None:
        ...
        assert DummyConcreteRound(self.base_synchronized_data, MagicMock()).is_majority_possible(
    ...
    def test_is_majority_possible_negative_case(self) -> None:
        ...
        assert not DummyConcreteRound(self.base_synchronized_data, MagicMock()).is_majority_possible(
            {
                "alice": DummyPayload("alice", False),
                "bob": DummyPayload("bob", True),
            },
...
class TestAbciApp:
    ...
    def test_initial_round_cls_not_set(self) -> None:
        ...
        with pytest.raises(ABCIAppInternalError, match="'initial_round_cls' field not set"):
    ...
    def test_transition_function_not_set(self) -> None:
        ...
        with pytest.raises(ABCIAppInternalError, match="'transition_function' field not set"):
    ...
    def test_process_event(self) -> None:
        ...
        assert self.abci_app._transition_backup.transition_function == AbciAppTest.transition_function
    ...
    def test_process_event_negative_case(self) -> None:
        ...
        with mock.patch.object(self.abci_app.logger, "warning") as mock_warning:
            ...
            mock_warning.assert_called_with("Cannot process event 'a' as current state is not set")
    ...
    def test_update_time(self) -> None:
        ...
        current_time += datetime.timedelta(0, AbciAppTest.TIMEOUT)
    ...
    def test_get_all_rounds_classes(
        ...
        actual_rounds = self.abci_app.get_all_round_classes({ConcreteBackgroundRound}, include_background_rounds)
    ...
    def test_get_all_rounds_classes_bg_ever_running(
        ...
        self.abci_app.background_apps.clear()
        self.abci_app.add_background_app(abci_base.BackgroundAppConfig(ConcreteBackgroundRound))
    ...
    def test_cleanup(self) -> None:
        ...
        max_participants = 4
        dummy_synchronized_data = BaseSynchronizedData(db=AbciAppDB(setup_data={"participants": [max_participants]}))
        ...
        assert list(self.abci_app.synchronized_data.db._data.keys()) == list(range(start_history_depth))
        ...
        history_len = len(self.abci_app.synchronized_data.db._data[reset_index]["dummy_key"])
...
class TestAvailabilityWindow:
    ...
    def test_add(max_length: int, hypothesis_data: Any) -> None:
        ...
        for _ in range(10):
            value = hypothesis_data.draw(booleans())
            expected_popped_value = None if max_length == 0 else availability_window._window[0]
            availability_window.add(value)
            assert len(availability_window._window) == max_length
    ...
    def test_validate_key(data_: dict, key: str, validator: Callable, expected_error: str | None) -> None:
...
def offence_tracking(draw: DrawFn) -> tuple[Evidences, LastCommitInfo]:
    ...
    unique_commit_addresses = {v.validator.address.decode() for v in commit_example.votes}
    ...
    assert unique_commit_addresses == {e.validator.address.decode() for e in ev_example.byzantine_validators}
...
def offence_status(draw: DrawFn) -> OffenceStatus:
    ...
    validator_downtime = just(AvailabilityWindow.from_dict(draw(availability_window_data())))
    invalid_payload = just(AvailabilityWindow.from_dict(draw(availability_window_data())))
...
class TestOffenseStatusEncoderDecoder:
    ...
    def test_encode_unknown(self) -> None:
        ...
        with pytest.raises(TypeError, match="Object of type Unknown is not JSON serializable"):
...
class TestRoundSequence:
    ...
    def setup(self) -> None:
        """Set up the test."""
        self.round_sequence = RoundSequence(context=MagicMock(), abci_app_cls=AbciAppTest)
    ...
    @given(data())
    def test_slashing_properties(self, property_name: str, set_twice_exc: str, config_exc: str, _data: Any) -> None:
        ...
        if value:
            ...
            with pytest.raises(
                ValueError,
                match=re.escape(set_twice_exc.format(new_content_attempt=new_content_attempt, value=value)),
            ):
    ...
    def test_sync_db_and_slashing(self, mock_loads: mock.MagicMock, slashing_config: str) -> None:
        ...
        mock_sync = cast(mock.Mock, self.round_sequence.abci_app.synchronized_data.db.sync)
        mock_sync.assert_called_once_with(serialized_db_state)

        if slashing_config:
            mock_loads.assert_called_once_with(slashing_config, cls=OffenseStatusDecoder)
        else:
            mock_loads.assert_not_called()

    @mock.patch("json.dumps")
    @pytest.mark.parametrize("slashing_enabled", [True, False])
    def test_store_offence_status(self, mock_dumps: mock.MagicMock, slashing_enabled: bool) -> None:
        ...
        if slashing_enabled:
            ...
            assert self.round_sequence.abci_app.synchronized_data.db.slashing_config == mock_encoded_status
            return
    ...
    def test_get_agent_address(self, validator: Validator, agent_address: str) -> None:
        ...
        round_sequence = RoundSequence(context=MagicMock(), abci_app_cls=AbciAppTest)
        round_sequence.validator_to_agent = {validator.address.hex().upper(): agent_address}
    ...
    def test_last_timestamp(self) -> None:
        ...
        seconds = 1
        nanoseconds = 1000
        expected_timestamp = datetime.datetime.fromtimestamp(seconds + nanoseconds / 10**9)
        self.round_sequence._blockchain.add_block(Block(MagicMock(height=1, timestamp=expected_timestamp), []))
    ...
    def test_last_round_transition_height(self, committed: bool) -> None:
        ...
        if committed:
            ...
            self.round_sequence.end_block()
            self.round_sequence.commit()
            assert self.round_sequence.last_round_transition_height == self.round_sequence._blockchain.height == 1
        else:
            assert self.round_sequence._blockchain.height == 0
            with pytest.raises(
                ValueError,
    ...
    def test_last_round_transition_root_hash(
        ...
        self.round_sequence._last_round_transition_root_hash = last_round_transition_root_hash
        ...
        if last_round_transition_root_hash == b"":
            ...
            assert self.round_sequence.last_round_transition_root_hash == last_round_transition_root_hash

    @pytest.mark.parametrize("tm_height", [None, 1, 5])
    def test_last_round_transition_tm_height(self, tm_height: int | None) -> None:
    ...
    def test_tm_height(self, tm_height: int) -> None:
        ...
        if tm_height is None:
            ...
            assert self.round_sequence.tm_height == self.round_sequence._tm_height == tm_height

    @given(one_of(none(), datetimes()))
    def test_block_stall_deadline_expired(self, block_stall_deadline: datetime.datetime) -> None:
    ...
    def test_init_chain(self, begin_height: int, initial_height: int) -> None:
        ...
        for i in range(begin_height):
            self.round_sequence._blockchain.add_block(MagicMock(header=MagicMock(height=i + 1)))
    ...
    def test_track_tm_offences(self, offences: tuple[Evidences, LastCommitInfo]) -> None:
        ...
        expected_offence_status = {
            dummy_addr_template.format(i=i): OffenceStatus() for i in range(len(last_commit_info.votes))
        }
        for i, vote_info in enumerate(last_commit_info.votes):
            ...
            expected_offence_status[agent_address].validator_downtime.add(expected_was_down)

        for byzantine_validator in evidences.byzantine_validators:
            agent_address = round_sequence._validator_to_agent[byzantine_validator.validator.address.hex()]
            evidence_type = byzantine_validator.evidence_type
            expected_offence_status[agent_address].num_unknown_offenses += bool(evidence_type == EvidenceType.UNKNOWN)
    ...
    def test_track_app_offences(self) -> None:
        ...
        self.round_sequence._offence_status = {dummy_addr_template.format(i=i): OffenceStatus() for i in range(4)}
    ...
    def test_handle_slashing_not_configured(self, exc: SlashingNotConfiguredError) -> None:
    ...
    def test_try_track_offences(self, _track_offences_raises: bool) -> None:
        ...
        with (
            ...
            if _track_offences_raises:
                ...
                _track_offences_mock.assert_called_once_with(evidences, last_commit_info)
    ...
    def test_deliver_tx_positive_not_valid(self) -> None:
        ...
        self.round_sequence.begin_block(MagicMock(), MagicMock(), MagicMock())
        with mock.patch.object(self.round_sequence.current_round, "check_transaction", return_value=True):
            with mock.patch.object(self.round_sequence.current_round, "process_transaction"):
                self.round_sequence.deliver_tx(MagicMock())
    ...
    def test_commit_negative_exception(self) -> None:
        ...
        self.round_sequence.begin_block(MagicMock(height=1), MagicMock(), MagicMock())
        self.round_sequence.end_block()
        with mock.patch.object(self.round_sequence._blockchain, "add_block", side_effect=AddBlockError):
    ...
    def test_commit_positive_with_change_round(self) -> None:
        ...
        with mock.patch.object(
            ...
            self.round_sequence.commit()
        assert not isinstance(self.round_sequence.abci_app._current_round, ConcreteRoundA)
    ...
    def test_update_round(
        ...
        end_block_res: tuple[BaseSynchronizedData, Any] | None,
        ...
        with mock.patch.object(self.round_sequence.current_round, "end_block", return_value=end_block_res):
        ...
        process_event_mock.assert_called_with(end_block_res[-1], result=end_block_res[0])
    ...
    def test_update_round_when_termination_returns(
        self,
        process_event_mock: mock.Mock,
        termination_round_result: tuple[BaseSynchronizedData, Any] | None,
        current_round_result: tuple[BaseSynchronizedData, Any] | None,
    ) -> None:
        """Test '_update_round' method."""
        self.round_sequence.begin_block(MagicMock(height=1), MagicMock(), MagicMock())
        block = self.round_sequence._block_builder.get_block()
        ...
        if termination_round_result is None and current_round_result is None:
            ...
            assert self.round_sequence._last_round_transition_height != self.round_sequence._blockchain.height
            assert self.round_sequence._last_round_transition_root_hash != self.round_sequence.root_hash
            assert self.round_sequence._last_round_transition_tm_height != self.round_sequence.tm_height
            process_event_mock.assert_not_called()
    ...
    def test_reset_state(
        ...
        with (
            ...
            mock.patch.object(self.round_sequence, "sync_db_and_slashing") as mock_sync_db_and_slashing,
        ):
            ...
            if restart_from_round in transition_fn:
                self.round_sequence.reset_state(round_id, round_count, serialized_db_state)
                ...
                if serialized_db_state is None:
                    ...
                    mock_sync_db_and_slashing.assert_called_once_with(serialized_db_state)
                    assert self.round_sequence._last_round_transition_root_hash == self.round_sequence.root_hash
...
class TestPendingOffencesRound(BaseRoundTestClass):
    ...
    @given(
        ...
        offense_type_value=sampled_from([value.value for value in OffenseType.__members__.values()]),
