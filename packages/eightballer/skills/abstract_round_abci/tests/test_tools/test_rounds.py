...
from packages.eightballer.skills.abstract_round_abci.tests.test_common import last_iteration
...
class DummyTxPayloadMatcher:
    ...
    def __repr__(self) -> str:
        ...
        return (
            "DummyTxPayload("
            f"id={self.expected.id_!r}, "
            f"round_count={self.expected.round_count!r}, "
            f"sender={self.expected.sender!r}, "
            f"value={self.expected.value!r}, "
            f"vote={self.expected.vote!r}"
            ")"
        )
...
class TestDummySynchronizedData:  # pylint: disable=too-few-public-methods
    ...
    def test_most_voted_keeper_address(
        ...
        dummy_synchronized_data = DummySynchronizedData(
            db=AbciAppDB(setup_data={most_voted_keeper_address_key: most_voted_keeper_address_data})
        )
        ...
        assert dummy_synchronized_data.most_voted_keeper_address == most_voted_keeper_address_data[-1]
...
class DummyCollectDifferentUntilAllRoundWithEndBlock(DummyCollectDifferentUntilAllRound):
    ...
    def end_block(self) -> tuple[BaseSynchronizedData, Enum] | None:
        ...
        if self.collection_threshold_reached and self.dummy_exit_event is not None:
            return (
                cast(
                    DummySynchronizedData,
                    self.synchronized_data.update(most_voted_keeper_address=list(self.collection.keys())),
                ),
...
class TestBaseCollectDifferentUntilAllRoundTest(BaseTestBase):
    ...
    def test_test_round(self, exit_event: DummyEvent) -> None:
        ...
        round_payloads = [DummyTxPayload(f"agent_{i}", str(i)) for i in range(MAX_PARTICIPANTS)]
        synchronized_data_attr_checks = [lambda _synchronized_data: _synchronized_data.most_voted_keeper_address]

        self.run_test(
            test_round=test_round,
            round_payloads=round_payloads,
            synchronized_data_update_fn=lambda synchronized_data, _: synchronized_data.update(
                most_voted_keeper_address=[f"agent_{i}" for i in range(MAX_PARTICIPANTS)]
            ),
    ...
    def end_block(self) -> tuple[BaseSynchronizedData, Enum] | None:
        ...
        if self.collection_threshold_reached:
            return (
                cast(
                    DummySynchronizedData,
                    self.synchronized_data.update(most_voted_keeper_address=self.common_payload),
                ),
                self.dummy_exit_event,
            )
        return None
...
class TestBaseCollectSameUntilAllRoundTest(BaseTestBase):
    ...
    base_round_test_cls: type[BaseCollectSameUntilAllRoundTest] = BaseCollectSameUntilAllRoundTest
    ...
    def test_test_round(self, exit_event: DummyEvent, common_value: str, finished: bool) -> None:
        """Test `_test_round`."""
        ...
        round_payloads = {f"test{i}": DummyTxPayload(f"agent_{i}", common_value) for i in range(MAX_PARTICIPANTS)}
        synchronized_data_attr_checks = [lambda _synchronized_data: _synchronized_data.most_voted_keeper_address]
...
class DummyCollectSameUntilThresholdRoundWithEndBlock(DummyCollectSameUntilThresholdRound):
    ...
    def end_block(self) -> tuple[BaseSynchronizedData, Enum] | None:
        ...
        if self.threshold_reached:
            return (
                cast(
                    DummySynchronizedData,
                    self.synchronized_data.update(most_voted_keeper_address=self.most_voted_payload),
                ),
                self.dummy_exit_event,
            )
...
class TestBaseCollectSameUntilThresholdRoundTest(BaseTestBase):
    ...
    base_round_test_cls: type[BaseCollectSameUntilThresholdRoundTest] = BaseCollectSameUntilThresholdRoundTest
    ...
    def test_test_round(self, exit_event: DummyEvent, most_voted_payload: str) -> None:
        ...
        round_payloads = {f"test{i}": DummyTxPayload(f"agent_{i}", most_voted_payload) for i in range(MAX_PARTICIPANTS)}
        synchronized_data_attr_checks = [lambda _synchronized_data: _synchronized_data.most_voted_keeper_address]
    ...
    def end_block(self) -> tuple[BaseSynchronizedData, Enum] | None:
        ...
        if self.keeper_payload is not None and any(val is not None for val in self.keeper_payload.values):
            return (
                cast(
                    DummySynchronizedData,
                    self.synchronized_data.update(blacklisted_keepers=self.keeper_payload.values[0]),
                ),
                self.dummy_exit_event,
            )
        return None
...
class TestBaseOnlyKeeperSendsRoundTest(BaseTestBase):
    ...
    base_round_test_cls: type[BaseOnlyKeeperSendsRoundTest] = BaseOnlyKeeperSendsRoundTest
    most_voted_keeper_address: str = "agent_0"

    def setup(self) -> None:
        """Setup that is run before each test."""
        super().setup()
        self.base_round_test.synchronized_data.update(most_voted_keeper_address=self.most_voted_keeper_address)
    ...
    def test_test_round(self, exit_event: DummyEvent, keeper_value: str) -> None:
        ...
        synchronized_data_attr_checks = [lambda _synchronized_data: _synchronized_data.blacklisted_keepers]
...
class TestBaseVotingRoundTest(BaseTestBase):
    ...
    def test_test_round(self, is_keeper_set: bool | None) -> None:
        """Test `_test_round`."""
        ...
        round_payloads = {
            f"test{i}": DummyTxPayload(f"agent_{i}", value="", vote=is_keeper_set) for i in range(MAX_PARTICIPANTS)
        }
        synchronized_data_attr_checks = [lambda _synchronized_data: _synchronized_data.is_keeper_set]
...
class DummyCollectDifferentUntilThresholdRoundWithEndBlock(DummyCollectDifferentUntilThresholdRound):
...
class TestBaseCollectDifferentUntilThresholdRoundTest(BaseTestBase):
    ...
    base_round_test_cls: type[BaseCollectDifferentUntilThresholdRoundTest] = BaseCollectDifferentUntilThresholdRoundTest
    ...
    def test_test_round(self, exit_event: DummyEvent) -> None:
        ...
        round_payloads = {f"test{i}": DummyTxPayload(f"agent_{i}", str(i)) for i in range(MAX_PARTICIPANTS)}
        synchronized_data_attr_checks = [lambda _synchronized_data: _synchronized_data.most_voted_keeper_address]
