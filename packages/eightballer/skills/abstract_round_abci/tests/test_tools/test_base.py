...
from packages.eightballer.skills.abstract_round_abci.tests.data.dummy_abci import PUBLIC_ID
from packages.eightballer.skills.abstract_round_abci.tests.test_tools.base import (
...
from packages.eightballer.skills.abstract_round_abci.tests.data.dummy_abci.behaviours import (
    DummyRoundBehaviour,
)
...
class TestFSMBehaviourBaseCaseSetup(FSMBehaviourTestToolSetup):
    ...
    def test_fast_forward_to_behaviour(self, behaviour: BaseBehaviour) -> None:
        ...
        synchronized_data = SynchronizedData(AbciAppDB(setup_data={"participants": [tuple("abcd")]}))
    ...
    def test_mock_ledger_api_request(self) -> None:
        ...
        request_kwargs = {"performative": LedgerApiMessage.Performative.GET_BALANCE}
        response_kwargs = {"performative": LedgerApiMessage.Performative.BALANCE}
        with pytest.raises(
            AssertionError,
    ...
    def test_mock_contract_api_request(self) -> None:
        ...
        request_kwargs = {"performative": ContractApiMessage.Performative.GET_STATE}
        response_kwargs = {"performative": ContractApiMessage.Performative.STATE}
        with pytest.raises(
            AssertionError,
            match="Invalid number of messages in outbox. Expected 1. Found 0.",
        ):
            test_instance.mock_contract_api_request(contract_id, request_kwargs, response_kwargs)
