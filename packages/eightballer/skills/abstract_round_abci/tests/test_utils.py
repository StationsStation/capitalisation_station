...
class TestVerifyDrand:
    ...
    def test_verify_fails(
        ...
        drand_value = DRAND_VALUE.copy()
        with mock.patch.object(self.drand_check, "_verify_signature", return_value=False):
            result, error = self.drand_check.verify(drand_value, DRAND_PUBLIC_KEY)
...
def test_get_data_from_nested_dict(nested_keys: list[str], final_value: bytes, separator: str) -> None:
...
def test_get_value_with_type(type_name: str, type_: type, value: Any) -> None:
    ...
    if type_ is None:
        with pytest.raises(AttributeError, match=f"module 'builtins' has no attribute '{type_name}'"):
...
def test_inverse(dict_: dict[KeyType, ValueType], expected: dict[ValueType, list[KeyType]]) -> None:
    """Test `inverse`."""
