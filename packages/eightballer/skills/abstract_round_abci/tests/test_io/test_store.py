...
class TestStorer:
    ...
    def test__get_single_storer_from_filetype(
        ...
        expected_storer: SupportedStorerType | None,
        tmp_path: PosixPath,
    ) -> None:
        """Test `_get_single_storer_from_filetype`."""
        if all(test_arg is None for test_arg in (filetype, custom_storer, expected_storer)):
            with pytest.raises(
                ValueError,
                match="Please provide either a supported filetype or a custom storing function.",
            ):
                Storer(filetype, custom_storer, str(tmp_path))._get_single_storer_from_filetype()

        else:
            expected_storer = cast(SupportedStorerType, expected_storer)
            storer = Storer(filetype, custom_storer, str(tmp_path))
            assert storer._get_single_storer_from_filetype().__code__.co_code == expected_storer.__code__.co_code
    ...
    def test_store_multiple(self) -> None:
        ...
        dummy_object = {"test": "test"}
        dummy_filename = "test"
        expected_path = str(Path(f"{self.path}/{dummy_filename}"))
