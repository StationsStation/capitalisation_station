...
from packages.eightballer.skills.abstract_round_abci.io_.store import SupportedFiletype
...
class TestLoader:
    ...
    def test__get_loader_from_filetype(
        ...
        expected_loader: SupportedLoaderType | None,
    ) -> None:
        """Test `_get_loader_from_filetype`."""
        if all(test_arg is None for test_arg in (filetype, custom_loader, expected_loader)):
            with pytest.raises(
                ValueError,
            ...
            assert loader._get_single_loader_from_filetype().__code__.co_code == expected_loader.__code__.co_code
