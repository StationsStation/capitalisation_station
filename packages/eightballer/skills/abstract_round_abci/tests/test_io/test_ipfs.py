...
from packages.eightballer.skills.abstract_round_abci.io_.load import AbstractLoader
...
class TestIPFSInteract:
    ...
    def test_store_and_send_and_back(
        ...
        serialized_objects = self.ipfs_interact.store(filepath, obj, multiple, SupportedFiletype.JSON)
    ...
    def test_store_fails(self, dummy_multiple_obj: dict[str, StoredJSONType]) -> None:
        ...
        with (
            ...
            self.ipfs_interact.store(dummy_filepath, dummy_multiple_obj, multiple, SupportedFiletype.JSON)
