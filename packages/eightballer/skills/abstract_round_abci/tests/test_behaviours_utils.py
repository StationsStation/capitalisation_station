...
from packages.valory.connections.ipfs.connection import PUBLIC_ID as IPFS_CONNECTION_ID, IpfsDialogues
from packages.valory.protocols.ledger_api.message import LedgerApiMessage
from packages.eightballer.skills.abstract_round_abci import behaviour_utils
...
from packages.eightballer.skills.abstract_round_abci.io_.ipfs import (
    IPFSInteract,
...
def yield_and_return_bool_wrapper(
    flag_value: bool,
) -> Callable[[], Generator[None, None, bool | None]]:
    """Wrapper for a Dummy generator that returns a `bool`."""
...
def yield_and_return_int_wrapper(
    value: int | None,
) -> Callable[[], Generator[None, None, int | None]]:
...
def test_async_behaviour_wait_for_message() -> None:
    ...
    class MyAsyncBehaviour(AsyncBehaviourTest):
        ...
        def async_act(self) -> Generator:
            self.counter += 1
            self.message = yield from self.wait_for_message(lambda message: message == expected_message)
            self.counter += 1
...
def test_async_behaviour_sleep() -> None:
    ...
    assert behaviour.counter == 2
    assert behaviour.state == AsyncBehaviour.AsyncState.READY
    assert behaviour.last_datetime is not None
    assert behaviour.first_datetime is not None
    assert (behaviour.last_datetime - behaviour.first_datetime).total_seconds() > timedelta
...
def _get_status_wrong_patch(*args: Any, **kwargs: Any) -> Generator[None, None, MagicMock]:
    """Patch `_get_status` method."""
    return MagicMock(body=json.dumps({"result": {"sync_info": {"latest_block_height": -1}}}).encode())
    yield


def _wait_until_transaction_delivered_patch(*args: Any, **kwargs: Any) -> Generator[None, None, tuple]:
...
class TestBaseBehaviour:
    ...
    def setup(self) -> None:
        ...
        self.context_mock.state.synchronized_data = self.context_state_synchronized_data_mock
        ...
        self.context_mock.ipfs_dialogues = IpfsDialogues(connection_id=str(IPFS_CONNECTION_ID))
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                MagicMock(ipfs_hash="test", performative=IpfsMessage.Performative.IPFS_HASH),
                "Successfully stored dummy_filename to IPFS with hash: test",
            ),
            (
                MagicMock(ipfs_hash="test", performative=IpfsMessage.Performative.ERROR),
    ...
    def test_send_to_ipfs(
        ...
        def dummy_do_ipfs_req(*args: Any, **kwargs: Any) -> Generator[None, None, IpfsMessage | None]:
        ...
        with (
            ...
            mock.patch.object(BaseBehaviour, "_do_ipfs_request", side_effect=dummy_do_ipfs_req) as do_req,
        ):
    ...
    def test_do_ipfs_request(self) -> None:
        ...
        message, dialogue = cast(IpfsDialogues, self.context_mock.ipfs_dialogues).create(
            str(IPFS_CONNECTION_ID), IpfsMessage.Performative.GET_FILES
        )
        message = cast(IpfsMessage, message)
        dialogue = cast(IpfsDialogue, dialogue)

        def dummy_wait_for_message(*args: Any, **kwargs: Any) -> Generator[None, None, Message]:
            """A dummy implementation of AsyncBehaviour.wait_for_message to be used for mocks."""
            return MagicMock()
            yield

        with mock.patch.object(AsyncBehaviour, "wait_for_message", side_effect=dummy_wait_for_message):
    ...
    def test_ipfs_get_fails(self, caplog: LogCaptureFixture) -> None:
        ...
        with (
            mock.patch.object(IPFSBehaviour, "_build_ipfs_get_file_req", side_effect=IPFSInteractionError),
            caplog.at_level(logging.ERROR),
        ):
            generator = self.behaviour.get_from_ipfs("dummy_ipfs_hash")
            try_send(generator)
    ...
    def test_synchronized_data_property(self) -> None:
        ...
        assert self.behaviour.synchronized_data == self.context_state_synchronized_data_mock
    ...
    def test_wait_until_round_end_negative_last_round_or_matching_round(self) -> None:
        ...
        self.behaviour.context.state.round_sequence.current_round_id = "current_round_id"
    ...
    def test_wait_from_last_timestamp(self) -> None:
        ...
        self.behaviour.context.state.round_sequence.abci_app.last_timestamp = last_timestamp
    ...
    def test_async_act_wrapper_agent_sync_mode(
        ...
        with (
            mock.patch.object(logging, "info") as log_mock,
            mock.patch.object(
                BaseBehaviour,
                "_get_status",
            ...
            for __ in range(3):
                try_send(gen)
            log_mock.assert_called_with("local height == remote == 0; Synchronization complete.")
    ...
    def test_send_transaction_stop_condition(self, *_: Any) -> None:
        ...
        self.behaviour.context.state.round_sequence.current_round_id = self.behaviour.matching_round.round_id = (
            "round_a"
        )
        ...
        stop_condition = self.behaviour.is_round_ended(self.behaviour.matching_round.auto_round_id())
        ...
        with (
            ...
            mock_error.assert_called_with(
                f"Received return code != 200 with response {response} with body {response.body!s}. "
            ...
            mock_debug.assert_called_with("Stop condition is true, no more attempts to send the transaction.")
    ...
    def test_send_transaction_positive_false_condition(self) -> None:
        ...
        with mock.patch.object(self.behaviour.context.logger, "debug") as mock_debug:
            try_send(self.behaviour._send_transaction(MagicMock(), stop_condition=lambda: True))
            mock_debug.assert_called_with("Stop condition is true, no more attempts to send the transaction.")
    ...
    def test_send_transaction_positive(self, *_: Any) -> None:
        ...
        success_response = MagicMock(status_code=200, body='{"result": {"tx_result": {"code": 0}}}')
    ...
    def test_tx_not_found(self, *_: Any) -> None:
        """Test _tx_not_found."""
        res = MagicMock(body='{"error": {"code": "dummy_code", "message": "dummy_message", "data": "dummy_data"}}')
    ...
    def test_send_transaction_timeout_exception_submit_tx(self, *_: Any) -> None:
        ...
        delay = 0.1
        m = MagicMock()
        with mock.patch.object(self.behaviour.context.logger, "warning") as mock_warning:
            gen = self.behaviour._send_transaction(m, request_timeout=timeout, request_retry_delay=delay)
            # trigger generator function
            try_send(gen, obj=None)
            try_send(gen, obj=m)
            time.sleep(timeout)
            try_send(gen, obj=m)
            time.sleep(delay)
            mock_warning.assert_called_with(f"Timeout expired for submit tx. Retrying in {delay} seconds...")
            try_send(gen, obj=None)
    ...
    @mock.patch.object(BaseBehaviour, "_check_http_return_code_200", return_value=True)
    def test_send_transaction_timeout_exception_wait_until_transaction_delivered(self, *_: Any) -> None:
        """Test '_send_transaction', timeout exception."""
        timeout = 0.05
        delay = 0.1
        m = MagicMock()
        with mock.patch.object(self.behaviour.context.logger, "warning") as mock_warning:
            gen = self.behaviour._send_transaction(m, request_retry_delay=delay, tx_timeout=timeout)
            # trigger generator function
            try_send(gen, obj=None)
            # send message to 'wait_for_message'
            try_send(gen, obj=m)
    ...
    def test_send_transaction_transaction_not_delivered(self, *_: Any) -> None:
        ...
        m = MagicMock()
        with mock.patch.object(self.behaviour.context.logger, "warning") as mock_warning:
            gen = self.behaviour._send_transaction(m, request_retry_delay=delay, tx_timeout=timeout, max_attempts=0)
            # trigger generator function
            try_send(gen, obj=None)
            # send message to 'wait_for_message'
            try_send(gen, obj=m)
            ...
            mock_warning.assert_called_with("Tx sent but not delivered. Response = None")
    ...
    def test_send_transaction_wrong_ok_code(self, *_: Any) -> None:
        ...
        with mock.patch.object(self.behaviour.context.logger, "error") as mock_error:
            ...
            mock_error.assert_called_with("Received tendermint code != 0. Retrying in 1.0 seconds...")
    ...
    def test_send_transaction_error_status_code(
        ...
        m = MagicMock()
        with mock.patch.object(self.behaviour.context.logger, "error") as mock_error:
            gen = self.behaviour._send_transaction(m, resetting, request_retry_delay=delay)
            # trigger generator function
            try_send(gen, obj=None)
            try_send(gen, obj=m)
            # send message to '_submit_tx'
            ...
            if resetting and non_200_count <= NON_200_RETURN_CODE_DURING_RESET_THRESHOLD:
                mock_error.assert_not_called()
            else:
                mock_error.assert_called_with(
                    f"Received return code != 200 with response {res} with body {res.body!s}. "
        ...
        [
            ...
            (
                ...
                None,
                {
                    ...
                    "signed_transaction": SignedTransaction(ledger_id="ethereum", body={"test_tx": "test_tx"}),
                },
            ),
        ],
    )
    def test_send_transaction_request(
        self,
        use_flashbots: bool,
        target_block_numbers: list[int] | None,
        expected_kwargs: Any,
    ) -> None:
        ...
        with mock.patch.object(
            ...
            self.behaviour._send_transaction_request(
                MagicMock(signed_transaction=SignedTransaction(ledger_id="ethereum", body={"test_tx": "test_tx"})),
                use_flashbots,
    ...
    def test_wait_until_transaction_delivered_failed(self, *_: Any) -> None:
        """Test '_wait_until_transaction_delivered' method."""
        gen = self.behaviour._wait_until_transaction_delivered(MagicMock(), max_attempts=0)
        # trigger generator function
        try_send(gen, obj=None)
        ...
        # second check attempt succeeds
        success_response = MagicMock(status_code=200, body='{"result": {"tx_result": {"code": -1}}}')
        try_send(gen, success_response)
    ...
    def test_send_raw_transaction(
        ...
        gen.send(
            SigningMessage(
                ...
                ("", ""),
                signed_transaction=SignedTransaction("ledger_id", body={"hash": expected_hash}),
            )
        )
        try:
            gen.send(ledger_message)
            msg = "Generator was expected to have reached its end!"
    ...
    @mock.patch.object(BaseBehaviour, "_send_transaction_receipt_request")
    @mock.patch.object(behaviour_utils, "Terms")
    def test_send_raw_transaction_with_wrong_signing_performative(self, *_: Any) -> None:
        """Test 'send_raw_transaction'."""
        m = MagicMock()
        gen = self.behaviour.send_raw_transaction(m)
        # trigger generator function
    ...
    def test_send_raw_transaction_errors(
        ...
        gen.send(
            SigningMessage(
                ...
                ("", ""),
                signed_transaction=SignedTransaction("ledger_id", body={"hash": "test"}),
            )
        )
        try:
            gen.send(
    ...
    def test_send_raw_transaction_hashes_mismatch(self, *_: Any) -> None:
        ...
        gen.send(
            SigningMessage(
                ...
                ("", ""),
                signed_transaction=SignedTransaction("ledger_id", body={"hash": "signed"}),
            )
        )
        try:
            gen.send(
    ...
    def test_get_contract_api_response(self, contract_address: str | None) -> None:
    ...
    @mock.patch.object(BaseBehaviour, "_build_http_request_message", return_value=(None, None))
    def test_get_status(self, _: mock.Mock) -> None:
        ...
        with mock.patch.object(BaseBehaviour, "_do_request", side_effect=dummy_do_request):
    ...
    def test_num_active_peers(
        self,
        num_peers: str | None,
        expected_num_peers: int | None,
        netinfo_status_code: int | None,
    ) -> None:
    ...
    def test_default_callback_request_stopped(self) -> None:
        ...
        with mock.patch.object(self.behaviour.context.logger, "debug") as debug_mock:
            ...
            debug_mock.assert_called_with("Dropping message as behaviour has stopped: %s", message)
    ...
    def test_default_callback_request_else(self, *_: Any) -> None:
        ...
        message = MagicMock()
        current_behaviour = self.behaviour
        with mock.patch.object(self.behaviour.context.logger, "warning") as warning_mock:
            self.behaviour.get_callback_request()(message, current_behaviour)
            warning_mock.assert_called_with("Could not send message to FSMBehaviour: %s", message)
    ...
    def test_acn_request_from_pending(
        ...
        self.behaviour.context.state.address_to_acn_deliverable = address_to_acn_deliverable
        ...
        with mock.patch.object(
            ...
            assert self.behaviour.context.outbox.put_message.call_count == len(dialogues_expected_calls)
            ...
            for i in range(self.behaviour.params.max_attempts):
                ...
                with mock.patch.object(
                    ...
                    try:
                        ...
                        msg = "The `_perform_acn_request` was expected to yield for the last time."
                        raise AssertionError(msg)

                    break

            n_expected_calls = final_attempt_idx + 1
            expected_calls = tuple(mock.call(performative) for _ in range(n_expected_calls))
    ...
    def test_request_recovery_params(self, expected_result: bool) -> None:
        ...
        with mock.patch.object(
            ...
            try:
                ...
                msg = "The `request_recovery_params` was expected to yield for the last time."
                raise AssertionError(msg)

            perform_acn_request_mock.assert_called_once_with(TendermintMessage.Performative.GET_RECOVERY_PARAMS)
    ...
    def test_get_reset_params(
        ...
        self.context_mock.state.round_sequence.last_round_transition_timestamp = timestamp
    ...
    def test_reset_tendermint_with_wait(
        ...
        reset_response: dict[str, bool | str] | str,
        status_response: dict[str, int | str] | str,
        local_height: int,
        on_startup: bool,
        n_iter: int,
        ...
        with (
            mock.patch.object(BaseBehaviour, "_is_timeout_expired", return_value=False),
            mock.patch.object(
                BaseBehaviour,
                "wait_from_last_timestamp",
                new_callable=lambda *_: dummy_generator_wrapper(),
            ),
            mock.patch.object(BaseBehaviour, "_do_request", new_callable=lambda *_: dummy_do_request),
            mock.patch.object(BaseBehaviour, "_get_status", new_callable=lambda *_: dummy_get_status),
            mock.patch.object(BaseBehaviour, "sleep", new_callable=lambda *_: dummy_generator_wrapper()),
        ):
            ...
            should_be_healthy = isinstance(reset_response, dict) and reset_response.get("status", False)
            ...
            try:
                ...
                if expecting_success:
                    ...
                    assert tm_recovery_params.round_count == shared_state.synchronized_data.db.round_count - 1
                    assert tm_recovery_params.reset_from_round == self.behaviour.matching_round.auto_round_id()
                    assert not self.behaviour._is_healthy
            else:
...
def test_degenerate_behaviour_async_act() -> None:
    ...
    behaviour = ConcreteDegenerateBehaviour(name=ConcreteDegenerateBehaviour.auto_behaviour_id(), skill_context=context)
    with pytest.raises(
        SystemExit,
    ):
        behaviour.act()
...
def test_make_degenerate_behaviour() -> None:
    ...
    assert new_cls.auto_behaviour_id() == f"degenerate_behaviour_{FinalRound.auto_round_id()}"
...
class TestTmManager:
    ...
    def setup(self) -> None:
        ...
        self.context_params_mock = MagicMock(
            ...
            tx_timeout=_DEFAULT_TX_TIMEOUT,
            max_attempts=_DEFAULT_TX_MAX_ATTEMPTS,
            consensus_params=MagicMock(consensus_threshold=self._DUMMY_CONSENSUS_THRESHOLD),
        )
        self.context_state_synchronized_data_mock = MagicMock(consensus_threshold=self._DUMMY_CONSENSUS_THRESHOLD)
        self.context_mock.params = self.context_params_mock
        self.context_mock.state.synchronized_data = self.context_state_synchronized_data_mock
    ...
    def test_handle_unhealthy_tm(
        ...
        def dummy_get_status(*_: Any) -> Generator[None, None, MagicMock]:
            ...
            yield
            return mock.MagicMock(
                body=json.dumps({"result": {"sync_info": {"latest_block_height": latest_block_height}}}).encode()
            )
        ...
        with (
            ...
            mock.patch.object(self.tm_manager, "sleep", side_effect=mock_sleep),
            mock.patch.object(
                BaseBehaviour,
    ...
    def test_handle_unhealthy_tm_logging(self, n_repetitions: int) -> None:
        ...
        def dummy_get_status(*_: Any) -> Generator[None, None, MagicMock]:
            """Dummy `_get_status` method."""
            yield
            return mock.MagicMock(body=json.dumps({"result": {"sync_info": {"latest_block_height": 0}}}).encode())
    ...
    def test_get_reset_params(self, expected_reset_params: dict[str, str] | None) -> None:
        ...
        def mock_handle_unhealthy_tm() -> Generator:
            ...
            for _ in range(num_iter):
                ...
                if msg is not None:
                    ...
                    self.tm_manager._AsyncBehaviour__state = AsyncBehaviour.AsyncState.RUNNING
            return
...
class TestIPFSBehaviour:
    ...
    def test_build_ipfs_store_file_req(self) -> None:
        ...
        with mock.patch.object(IPFSInteract, "store", return_value=MagicMock()) as mock_store:
    ...
    def test_deserialize_ipfs_objects(self) -> None:
        ...
        with mock.patch.object(IPFSInteract, "load", return_value=MagicMock()) as mock_load:
