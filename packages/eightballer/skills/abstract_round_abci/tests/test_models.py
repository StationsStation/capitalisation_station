...
from packages.eightballer.skills.abstract_round_abci.tests.conftest import (
    irrelevant_genesis_config,
)
from packages.eightballer.skills.abstract_round_abci.test_tools.abci_app import AbciAppTest
...
class TestApiSpecsModel:
    ...
    def test_suggested_sleep_time(self, retries: int) -> None:
        ...
        assert self.api_specs.retries_info.suggested_sleep_time == DEFAULT_BACKOFF_FACTOR**retries
    ...
    def test_get_spec(
        ...
        assert all(key in specs for key in actual_specs)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                MagicMock(body=b'{"test": {"response": {"key": ["does_not_matter", "does_not_matter"]}}}'),
                None,
                None,
            ),
            (
    ...
    def test_process_response(
        ...
        response_type = api_specs_config.get("response_type")
...
class TestSharedState:
    ...
    def test_setup_slashing(
        ...
        if not raises:
            ...
            expected_status = {agent: OffenceStatus() for agent in acn_configured_agents}
            encoded_expected_status = json.dumps(expected_status, cls=OffenseStatusEncoder)
        ...
        expected_diff = acn_configured_agents.symmetric_difference(validator_to_agent.values())
        with pytest.raises(
            ValueError,
            match=re.escape(
    ...
    def test_setup(self, *_: Any) -> None:
        """Test setup method."""
        shared_state = SharedState(name="", skill_context=MagicMock(is_abstract_component=False))
        assert shared_state.initial_tm_configs == {}
        self.dummy_state_setup(shared_state)
        assert shared_state.initial_tm_configs == dict.fromkeys(range(4))
    ...
    def test_get_validator_address(
        self,
        initial_tm_configs: dict[str, dict[str, Any] | None],
        address_input: str,
        exception: str | None,
        expected: str | None,
    ) -> None:
    ...
    def test_acn_container(self, self_idx: int) -> None:
        ...
        shared_state = SharedState(name="", skill_context=MagicMock(agent_address=self_idx))
        self.dummy_state_setup(shared_state)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ({}, 4, None),
            (dict.fromkeys(range(4), "test"), 4, "test"),
            (
    ...
    def test_get_acn_result(
        ...
        """Test `get_acn_result`."""
        shared_state = SharedState(abci_app_cls=AbciAppTest, name="", skill_context=MagicMock())
        shared_state.context.params.setup_params = {
            "test": [],
            "all_participants": ["0x0"],
        }
    ...
    def test_recovery_params_on_init(self) -> None:
        ...
        assert shared_state.tm_recovery_params.reset_from_round == AbciAppTest.initial_round_cls.auto_round_id()
...
class TestBenchmarkTool:
    ...
    def _check_behaviour_data(data: list, agent_name: str) -> None:
        ...
        assert all(key in behaviour_data["data"] for key in ("local", "consensus", "total"))

    def test_end_2_end(self) -> None:
        """Test end 2 end of the tool."""

        agent_name = "agent"
        skill_context = MagicMock(agent_address=agent_name, logger=MagicMock(info=logging.info))

        with TemporaryDirectory() as temp_dir:
            benchmark = BenchmarkTool(name=agent_name, skill_context=skill_context, log_dir=temp_dir)
...
def test_genesis_validator() -> None:
    ...
    with pytest.raises(TypeError, match="Error in field 'pub_key_types'. Expected type .*"):
...
def test_shared_state_instantiation_without_attributes_raises_error() -> None:
    ...
    with pytest.raises(
        AttributeError,
        match="The class <class 'unittest.mock.MagicMock'> is not an instance of packages.eightballer.skills.abstract_round_abci.base.AbciApp",
    ):
...
testdata_positive = [
    ...
    ("test_arg", [1], list[int | None]),
    ("test_arg", [1, None], list[int | None]),
    ("test_arg", A, type[A]),
...
testdata_negative = [
    ...
    ("test_arg", ["1"], list[int | None]),
    ("test_arg", [1, None, "1"], list[int | None]),
    ("test_arg", B, type[A]),
