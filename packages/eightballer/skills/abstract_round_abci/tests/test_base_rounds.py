...
class TestCollectionRound(_BaseRoundTestClass):
    ...
    def setup(
        ...
        self.test_round = DummyCollectionRound(synchronized_data=self.synchronized_data, context=MagicMock())
    ...
    def test_serialized_collection(self) -> None:
        ...
        mcs_key = "packages.eightballer.skills.abstract_round_abci.test_tools.rounds.DummyTxPayload"
        expected = {
            f"agent_{i}": {
    ...
    def test_run(
        ...
        assert self.test_round.accepting_payloads_from == self.synchronized_data.participants
        self.test_round._allow_rejoin_payloads = True
        assert self.test_round.accepting_payloads_from == self.synchronized_data.all_participants
        ...
        with pytest.raises(
            TransactionNotValidError,
            match=re.escape("sender not in list of participants: ['agent_0', 'agent_1', 'agent_2', 'agent_3']"),
        ):
...
class TestCollectSameUntilThresholdRound(_BaseRoundTestClass):
    ...
    def test_run(
        ...
        first_payload, *payloads = get_dummy_tx_payloads(self.participants, value="vote")
        test_round.process_payload(first_payload)
        ...
        payloads = get_dummy_tx_payloads(self.participants, value=None, is_value_none=True, is_vote_none=True)
        for payload in payloads:
            test_round.process_payload(payload)
...
class TestOnlyKeeperSendsRound(_BaseRoundTestClass, BaseOnlyKeeperSendsRoundTest):
    ...
    @pytest.mark.parametrize("payload_key", ["dummy_key", tuple(f"dummy_key_{i}" for i in range(2))])
    def test_run(
        self,
        ...
        test_round = DummyOnlyKeeperSendsRound(
            synchronized_data=self.synchronized_data.update(most_voted_keeper_address="agent_0"),
            context=MagicMock(),
        )
        ...
        with pytest.raises(ABCIAppInternalError, match="internal error: agent_1 not elected as keeper."):
            test_round.process_payload(DummyTxPayload(sender="agent_1", value="sender"))

        with pytest.raises(TransactionNotValidError, match="keeper payload value already set."):
            test_round.check_payload(first_payload)
        ...
        with pytest.raises(TransactionNotValidError, match="agent_1 not elected as keeper."):
...
class TestVotingRound(_BaseRoundTestClass):
    ...
    def test_threshold(self, vote: bool | None) -> None:
        """Runs threshold test."""
...
class TestCollectDifferentUntilThresholdRound(_BaseRoundTestClass):
    ...
    @pytest.mark.parametrize("required_confirmations", [MAX_PARTICIPANTS, MAX_PARTICIPANTS + 1])
    def test_run(
        self,
...
class TestCollectNonEmptyUntilThresholdRound(_BaseRoundTestClass):
    ...
    def test_get_non_empty_values(self) -> None:
        ...
        assert non_empty_values == {
            tuple(sorted(self.participants))[i]: ((f"agent_{i}", False) if i != none_payload_idx else (False,))
            for i in range(4)
        }
    ...
    def test_end_block(
        ...
        test_round.selection_key = selection_key
        payloads = get_dummy_tx_payloads(self.participants, is_value_none=is_value_none, is_vote_none=True)
        for payload in payloads:
            test_round.process_payload(payload)
