...
from packages.eightballer.skills.abstract_round_abci.models import BaseParams
from packages.eightballer.skills.abstract_round_abci.tests.conftest import irrelevant_config
...
def test_random_selection() -> None:
    ...
    with pytest.raises(ValueError, match=re.escape("Randomness should lie in the [0,1) interval")):
...
def dummy_generator(
    ...
    def dummy_generator_wrapped(*_: Any, **__: Any) -> Generator[None, None, ReturnValueType]:
...
class TestRandomnessBehaviour(BaseDummyBehaviour):
    ...
    def test_failsafe_randomness(self, return_value: MagicMock, expected_hash: str | None) -> None:
        ...
        with mock.patch.object(
            ...
            try:
                ...
                msg = "`get_ledger_api_response`'s generator should have been exhausted."
                raise AssertionError(msg)

    @pytest.mark.parametrize("randomness_response", ["test", None])
    @pytest.mark.parametrize("verified", [True, False])
    def test_get_randomness_from_api(self, randomness_response: str | None, verified: bool) -> None:
        ...
        with (
            ...
            try:
                ...
                msg = "`get_randomness_from_api`'s generator should have been exhausted."
                raise AssertionError(msg)
    ...
    def test_async_act(
        ...
        self.behaviour.context.randomness_api.is_retries_exceeded = lambda: retries_exceeded
        ...
        with (
            ...
            ) as send_a2a_transaction_mocked,
            mock.patch.object(
                self.behaviour,
                "wait_until_round_end",
            ) as wait_until_round_end_mocked,
            mock.patch.object(
                self.behaviour,
                "set_done",
            ) as set_done_mocked,
