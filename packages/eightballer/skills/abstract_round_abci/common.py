...
from packages.eightballer.skills.abstract_round_abci.base import BaseTxPayload
from packages.eightballer.skills.abstract_round_abci.utils import VerifyDrand
from packages.eightballer.skills.abstract_round_abci.behaviour_utils import BaseBehaviour


RandomnessObservation = Optional[dict[str, str | int]]
...
def random_selection(elements: list[Any], randomness: float) -> str:
    ...
    """
    if not elements:
        msg = "No elements to randomly select among"
        raise ValueError(msg)
    if randomness < 0 or randomness >= 1:
        msg = "Randomness should lie in the [0,1) interval"
        raise ValueError(msg)
...
class RandomnessBehaviour(BaseBehaviour, ABC):
    ...
    def failsafe_randomness(
        ...
        randomness = hashlib.sha256(
            cast(str, ledger_api_response.state.body.get("hash")).encode() + str(self.params.service_id).encode()
        ).hexdigest()
    ...
    def async_act(self) -> Generator:
        ...
        with self.context.benchmark_tool.measure(self.behaviour_id).local():
            if self.context.randomness_api.is_retries_exceeded():
                ...
                if observation is None:
                    self.context.logger.error("Could not generate randomness from chain!")
                    return
            else:
        ...
        if observation:
            ...
            self.context.logger.error(f"Could not get randomness from {self.context.randomness_api.api_id}")
            yield from self.sleep(self.context.randomness_api.retries_info.suggested_sleep_time)
...
class SelectKeeperBehaviour(BaseBehaviour, ABC):
    ...
    def _select_keeper(self) -> str:
        ...
        non_blacklisted = self.synchronized_data.participants - self.synchronized_data.blacklisted_keepers
        if not non_blacklisted:
            msg = "Cannot continue if all the keepers have been blacklisted!"
            raise RuntimeError(msg)

        # Sorted list of participants who are not blacklisted as keepers
        relevant_set = sorted(non_blacklisted)
        ...
        if self.synchronized_data.is_keeper_set and len(self.synchronized_data.participants) > 1:
            old_keeper_index = relevant_set.index(self.synchronized_data.most_voted_keeper_address)
