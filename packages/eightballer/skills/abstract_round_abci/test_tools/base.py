...
class FSMBehaviourBaseCase(BaseSkillTestCase, ABC):
    ...
    def setup_class(cls, **kwargs: Any) -> None:
        ...
        if not hasattr(cls, "path_to_skill"):
            msg = f"No `path_to_skill` set on {cls}"
        ...
        assert cls._skill.skill_context._agent_context is not None, "Agent context not set"  # nosec
        cls._skill.skill_context._agent_context.identity._default_address_key = cls.default_ledger
        ...
        assert isinstance(behaviour, AbstractRoundBehaviour), f"{behaviour} is not of type {AbstractRoundBehaviour}"
        ...
        for attr, handler, handler_type in [
            ...
            assert isinstance(handler, handler_type), f"{handler} is not of type {handler_type}"
    ...
    def setup(self, **kwargs: Any) -> None:
        ...
        self._skill.skill_context.benchmark_tool.__dict__["log_dir"] = Path(self.benchmark_dir.name)
        assert (  # nosec
    ...
    def fast_forward_to_behaviour(
        ...
        next_behaviour = {s.auto_behaviour_id(): s for s in behaviour.behaviours}[behaviour_id]
        ...
        self.skill.skill_context.state.round_sequence.abci_app._round_results.append(synchronized_data)
        ...
        self.skill.skill_context.state.round_sequence.abci_app._current_round_cls = next_behaviour.matching_round
        # consensus parameters will not be available if the current skill is abstract
        consensus_params = getattr(self.skill.skill_context.params, "consensus_params", None)
        self.skill.skill_context.state.round_sequence.abci_app._current_round = next_behaviour.matching_round(
            synchronized_data, consensus_params
        )

    def mock_ledger_api_request(self, request_kwargs: dict, response_kwargs: dict) -> None:
        """Mock http request.
    ...
    def mock_contract_api_request(self, contract_id: str, request_kwargs: dict, response_kwargs: dict) -> None:
        """Mock http request.
    ...
    def mock_a2a_transaction(
        ...
        self.mock_http_request(
            ...
            response_kwargs={
                ...
                "status_text": "",
                "headers": "",
                "body": json.dumps({"result": {"hash": "", "code": OK_CODE}}).encode("utf-8"),
            },
        )
        self.mock_http_request(
            request_kwargs={
            ...
            response_kwargs={
                ...
                "status_text": "",
                "headers": "",
                "body": json.dumps({"result": {"tx_result": {"code": OK_CODE}}}).encode("utf-8"),
            },
        )
    ...
    def end_round(self, done_event: Enum) -> None:
        ...
        abci_app._current_round = abci_app.transition_function[current_behaviour.matching_round][done_event](
            abci_app.synchronized_data, context=MagicMock()
        )
    ...
    def _test_done_flag_set(self) -> None:
        ...
        with mock.patch.object(self.behaviour.context.state, "_round_sequence") as mock_round_sequence:
            mock_round_sequence.last_round_id = cast(AbstractRound, current_behaviour.matching_round).auto_round_id()
