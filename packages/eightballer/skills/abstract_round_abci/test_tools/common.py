...
class BaseRandomnessBehaviourTest(CommonBaseCase):
    ...
    def test_invalid_response(
        ...
        self.mock_http_request(
            ...
            response_kwargs={"version": "", "status_code": 200, "status_text": "", "headers": "", "body": b""},
        )
        self.behaviour.act_wrapper()
        time.sleep(1)
        self.behaviour.act_wrapper()
    ...
    def test_max_retries_reached_fallback(
        ...
        with mock.patch.object(
            ...
            self.mock_ledger_api_request(
                request_kwargs={"performative": LedgerApiMessage.Performative.GET_STATE},
                response_kwargs={
...
class BaseSelectKeeperBehaviourTest(CommonBaseCase):
    ...
    def test_select_keeper(self, agent_address_mock: mock.Mock, blacklisted_keepers: set[str]) -> None:
        ...
        assert self.behaviour.current_behaviour is not None
        assert self.behaviour.current_behaviour.behaviour_id == self.select_keeper_behaviour_class.auto_behaviour_id()
        ...
        if (
            ...
            self._test_done_flag_set()
            self.end_round(self.done_event)
            assert self.behaviour.current_behaviour.behaviour_id == self.next_behaviour_class.auto_behaviour_id()
        else:
            with pytest.raises(
                AEAActException,
