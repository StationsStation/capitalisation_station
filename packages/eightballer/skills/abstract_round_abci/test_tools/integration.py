...
from packages.eightballer.skills.abstract_round_abci.base import BaseSynchronizedData
from packages.eightballer.skills.abstract_round_abci.handlers import SigningHandler
from packages.eightballer.skills.abstract_round_abci.behaviour_utils import BaseBehaviour
...
class IntegrationBaseCase(FSMBehaviourBaseCase, ABC):
    ...
    def setup_class(cls, **kwargs: Any) -> None:
        ...
        cls.multiplexer = Multiplexer([cls.make_ledger_api_connection_callable()], loop=cls.running_loop)
        ...
        cls.decision_maker = DecisionMaker(decision_maker_handler=DecisionMakerHandler(identity, wallet, {}))
        cls._skill._skill_context._agent_context._decision_maker_message_queue = cls.decision_maker.message_in_queue
    ...
    def get_message_from_decision_maker_inbox(self) -> Message | None:
    ...
    def process_message_cycle(
        ...
        """
        if expected_types and next(iter(expected_types)) == "transaction_receipt":
    ...
    def process_n_messages(
        self,
        ncycles: int,
        synchronized_data: BaseSynchronizedData | None = None,
        behaviour_id: str | None = None,
        handlers: HandlersType | None = None,
        expected_content: ExpectedContentType | None = None,
        expected_types: ExpectedTypesType | None = None,
        ...
        expected_content = [None] * ncycles if expected_content is None else expected_content
        ...
        if behaviour_id is not None and synchronized_data is not None:
            ...
            assert (  # nosec
                cast(BaseBehaviour, self.behaviour.current_behaviour).behaviour_id == behaviour_id
            )
...
class HardHatHelperIntegration(IntegrationBaseCase, ABC):  # pragma: no cover
    ...
    def setup_class(cls, **kwargs: Any) -> None:
        ...
        cls.hardhat_provider = Web3(provider=HTTPProvider("http://localhost:8545")).provider
