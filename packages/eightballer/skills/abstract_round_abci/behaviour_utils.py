...
from packages.eightballer.skills.abstract_round_abci.io_.load import Loader, CustomLoaderType
...
from packages.eightballer.connections.p2p_libp2p_client.connection import (
...
class AsyncBehaviour(ABC):
    ...
    def __init__(self) -> None:
        ...
        self.__generator_act: Generator | None = None
    ...
    def wait_for_condition(
        cls, condition: Callable[[], bool], timeout: float | None = None
...
class IPFSBehaviour(SimpleBehaviour, ABC):
    ...
    def _build_ipfs_store_file_req(  # pylint: disable=too-many-arguments
        ...
        **kwargs: Any,
        ...
        """
        serialized_objects = self._ipfs_interact.store(filename, obj, multiple, filetype, custom_storer, **kwargs)
    ...
    ) -> SupportedObjectType | None:
        """Deserialize objects received from IPFS."""
        return self._ipfs_interact.load(serialized_objects, filetype, custom_loader)
...
class _MetaBaseBehaviour(ABCMeta):
    ...
    def __new__(cls, name: str, bases: tuple, namespace: dict, **kwargs: Any) -> type:  # type: ignore
        ...
        cls._check_consistency(cast(type[BaseBehaviour], new_cls))
        return new_cls

    @classmethod
    def _check_consistency(cls, base_behaviour_cls: type["BaseBehaviour"]) -> None:
        """Check consistency of class attributes."""
        cls._check_required_class_attributes(base_behaviour_cls)

    @classmethod
    def _check_required_class_attributes(cls, base_behaviour_cls: type["BaseBehaviour"]) -> None:
        """Check that required class attributes are set."""
        if not hasattr(base_behaviour_cls, "matching_round"):
            msg = f"'matching_round' not set on {base_behaviour_cls}"
            raise BaseBehaviourInternalError(msg)


class BaseBehaviour(AsyncBehaviour, IPFSBehaviour, CleanUpBehaviour, ABC, metaclass=_MetaBaseBehaviour):
    ...
    def __init__(self, **kwargs: Any):  # pylint: disable=super-init-not-called
        ...
        self._check_started: datetime.datetime | None = None
    ...
    def auto_behaviour_id(cls) -> str:
        ...
        """
        return cls.behaviour_id if isinstance(cls.behaviour_id, str) else cls.__pattern.sub("_", cls.__name__).lower()
    ...
    def wait_until_round_end(self, timeout: float | None = None) -> Generator[None, None, None]:
    ...
    def wait_from_last_timestamp(self, seconds: float) -> Any:
        ...
        """
        if seconds < 0:
            msg = "Can only wait for a positive amount of time"
            raise ValueError(msg)
    ...
    def send_a2a_transaction(self, payload: BaseTxPayload, resetting: bool = False) -> Generator:
        ...
        for _ in range(self.context.params.tendermint_max_retries):
            ...
            try:
                ...
                if _is_sync_complete:
                    self.context.logger.info(f"local height == remote == {local_height}; Synchronization complete.")
                ...
                self.context.logger.debug("Tendermint not accepting transactions yet, trying again!")
                yield from self.sleep(self.context.params.tendermint_check_sleep_delay)
    ...
    def _send_transaction(  # pylint: disable=too-many-arguments,too-many-locals,too-many-statements
        ...
        :yield: the responses
        """
        request_timeout = self.params.request_timeout if request_timeout is None else request_timeout
        request_retry_delay = self.params.request_retry_delay if request_retry_delay is None else request_retry_delay
        tx_timeout = self.params.tx_timeout if tx_timeout is None else tx_timeout
        max_attempts = self.params.max_attempts if max_attempts is None else max_attempts
        while not stop_condition():
            self.context.logger.debug(f"Trying to send payload: {pprint.pformat(payload.json)}")
            signature_bytes = yield from self.get_signature(payload.encode())
            transaction = Transaction(payload, signature_bytes)
            try:
                response = yield from self._submit_tx(transaction.encode(), timeout=request_timeout)
            ...
            if non_200_code and (
                self._non_200_return_code_count > NON_200_RETURN_CODE_DURING_RESET_THRESHOLD or not resetting
            ):
            ...
            try:
                ...
                msg = f"Unable to decode response: {response} with body {response.body!s}"
            ...
            if isinstance(res, HttpMessage) and self._tx_not_found(tx_hash, res):
                ...
                self.context.logger.warning(f"Tx sent but not delivered. Response = {res}")
            payload = payload.with_new_id()
        self.context.logger.debug("Stop condition is true, no more attempts to send the transaction.")
    ...
    def _is_invalid_transaction(res: HttpMessage) -> bool:
        ...
        try:
            ...
            return any(error_code in body_["tx_result"]["info"] for error_code in error_codes)
    ...
    def _tx_not_found(tx_hash: str, res: HttpMessage) -> bool:
        ...
        try:
            ...
            return all(text == error[field] for field, text in not_found_field_to_text.items())
        except Exception:  # pylint: disable=broad-except  # pragma: nocover
            return False

    def _send_signing_request(self, raw_message: bytes, is_deprecated_mode: bool = False) -> None:
        ...
        cast(Requests, self.context.requests).request_id_to_callback[request_nonce] = self.get_callback_request()
        self.context.decision_maker_message_queue.put_nowait(signing_msg)

    def _send_transaction_signing_request(self, raw_transaction: RawTransaction, terms: Terms) -> None:
    ...
    def _send_transaction_request(
        ...
        target_block_numbers: list[int] | None = None,
        ...
        create_kwargs: dict[str, str | SignedTransaction | list[SignedTransaction]] = {
            "counterparty": LEDGER_API_ADDRESS,
            "performative": LedgerApiMessage.Performative.SEND_SIGNED_TRANSACTION,
        }
        if chain_id:
            kwargs = LedgerApiMessage.Kwargs({"chain_id": chain_id})
            create_kwargs.update({"kwargs": kwargs})
        ...
        ledger_api_msg, ledger_api_dialogue = ledger_api_dialogues.create(**create_kwargs)
    ...
    def _send_transaction_receipt_request(
        ...
        **kwargs: Any,
        ...
        cast(Requests, self.context.requests).request_id_to_callback[request_nonce] = self.get_callback_request()
        self.context.outbox.put_message(message=ledger_api_msg)
        self.context.logger.debug(f"Sending transaction receipt request for tx_digest='{tx_digest}'...")

    def _handle_signing_failure(self) -> None:
        """Handle signing failure."""
        self.context.logger.error("The transaction could not be signed!")

    def _submit_tx(self, tx_bytes: bytes, timeout: float | None = None) -> Generator[None, None, HttpMessage]:
        ...
        request_message, http_dialogue = self._build_http_request_message(
            "GET",
            self.context.params.tendermint_url + f"/broadcast_tx_sync?tx=0x{tx_bytes.hex()}",
        )
        result = yield from self._do_request(request_message, http_dialogue, timeout=timeout)
        return result

    def _get_tx_info(self, tx_hash: str, timeout: float | None = None) -> Generator[None, None, HttpMessage]:
    ...
    def _get_netinfo(self, timeout: float | None = None) -> Generator[None, None, HttpMessage]:
    ...
    def num_active_peers(self, timeout: float | None = None) -> Generator[None, None, int | None]:
        ...
        try:
            ...
            if http_response.status_code != http_ok:
                ...
                self.context.logger.warning(f"`/net_info` responded with status {http_response.status_code}.")
                return None
            ...
            self.context.logger.warning(f"Couldn't retrieve `/net_info` response in {timeout}s.")
            return None
    ...
    def get_callback_request(self) -> Callable[[Message, "BaseBehaviour"], None]:
        ...
        def callback_request(message: Message, current_behaviour: BaseBehaviour) -> None:
            ...
            if self.is_stopped:
                self.context.logger.debug("Dropping message as behaviour has stopped: %s", message)
                ...
                self.context.logger.warning("Could not send message to FSMBehaviour: %s", message)
    ...
    def _wait_until_transaction_delivered(
        ...
        max_attempts: int | None = None,
        request_retry_delay: float | None = None,
    ) -> Generator[None, None, tuple[bool, HttpMessage | None]]:
        ...
        for _ in range(max_attempts):
            request_timeout = (deadline - datetime.datetime.now()).total_seconds() if timeout is not None else None
    ...
    def get_signature(self, message: bytes, is_deprecated_mode: bool = False) -> Generator[None, None, str]:
        ...
        if signature_response.performative == SigningMessage.Performative.ERROR:
            ...
            msg = "Internal error: failure during signing."
    ...
    def send_raw_transaction(
        ...
        None,
        None | SigningMessage | LedgerApiMessage,
        tuple[str | None, RPCResponseStatus],
    ]:
        ...
        self.context.logger.info(f"Sending signing request to ledger '{chain_id}' for transaction: {transaction}...")
        ...
        if signature_response.performative != SigningMessage.Performative.SIGNED_TRANSACTION:
            self.context.logger.error(f"Error when requesting transaction signature: {signature_response}")
        ...
        self.context.logger.info(f"Transaction sent! Received transaction digest: {tx_hash}")
    ...
    def get_transaction_receipt(
        ...
        if chain_id is None:
            chain_id = self.params.default_chain_id
        self._send_transaction_receipt_request(tx_digest, retry_timeout, retry_attempts, chain_id=chain_id)
        transaction_receipt_msg = yield from self.wait_for_message()
        if transaction_receipt_msg.performative == LedgerApiMessage.Performative.ERROR:  # pragma: nocover
            self.context.logger.error(f"Error when requesting transaction receipt: {transaction_receipt_msg.message}")
            return None
    ...
    def get_contract_api_response(
        ...
        **kwargs: Any,
        ...
        contract_api_msg, contract_api_dialogue = contract_api_dialogues.create(**kwargs)
    ...
    def _acn_request_from_pending(self, performative: TendermintMessage.Performative) -> Generator:
    ...
    def _perform_acn_request(self, performative: TendermintMessage.Performative) -> Generator[None, None, Any]:
        ...
        for i in range(self.params.max_attempts):
            self.context.logger.debug(f"ACN attempt {i + 1}/{self.params.max_attempts}.")
    ...
    def _is_timeout_expired(self) -> bool:
        ...
        if self._check_started is None or self._is_healthy:
            return False
        return datetime.datetime.now() > self._check_started + datetime.timedelta(0, self._timeout)

    def _get_reset_params(self, default: bool) -> dict[str, str] | None:
        """Get the parameters for a hard reset request to Tendermint."""
        if default:
            return None

        last_round_transition_timestamp = self.round_sequence.last_round_transition_timestamp
        genesis_time = last_round_transition_timestamp.astimezone(pytz.UTC).strftime(GENESIS_TIME_FMT)
        return {
    ...
    def send_to_ipfs(  # pylint: disable=too-many-arguments
        ...
        **kwargs: Any,
        ...
        try:
            ...
            self.context.logger.info(f"Successfully stored {filename} to IPFS with hash: {ipfs_hash}")
            return ipfs_hash
        except IPFSInteractionError as e:  # pragma: no cover
            self.context.logger.exception(f"An error occurred while trying to send a file to IPFS: {e!s}")
            return None
    ...
    def get_from_ipfs(  # pylint: disable=too-many-arguments
        ...
        custom_loader: CustomLoaderType = None,
        timeout: float | None = None,
    ) -> Generator[None, None, SupportedObjectType | None]:
        ...
        try:
            ...
            deserialized_objects = self._deserialize_ipfs_objects(serialized_objects, filetype, custom_loader)
            self.context.logger.info(f"Retrieved {len(ipfs_message.files)} objects from ipfs.")
            return deserialized_objects
        except IPFSInteractionError as e:
            self.context.logger.exception(f"An error occurred while trying to fetch a file from IPFS: {e!s}")
            return None
...
class TmManager(BaseBehaviour):
    ...
    _active_generator: Generator | None = None
    ...
    def get_callback_request(self) -> Callable[[Message, "BaseBehaviour"], None]:
        ...
        def callback_request(message: Message, _current_behaviour: BaseBehaviour) -> None:
            ...
            if self.state == AsyncBehaviour.AsyncState.WAITING_MESSAGE:
                ...
                self.context.logger.warning("Could not send message to TmManager: %s", message)
...
class DegenerateBehaviour(BaseBehaviour, ABC):
    ...
    def async_act(self) -> Generator:
        ...
        self.context.logger.error(f"Sleeping {self.sleep_time_before_exit} seconds before exiting.")
