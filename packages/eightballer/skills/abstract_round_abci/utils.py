...
class VerifyDrand:  # pylint: disable=too-few-public-methods
    ...
    def _int_to_bytes_big(cls, value: int) -> bytes:
        ...
        if value < 0 or value > MAX_UINT64:
            msg = "VerifyDrand can only handle positive numbers representable with 8 bytes"
    ...
    def _verify_signature(
        ...
        message: bytes,
        signature: BLSSignature | bytes,
    ) -> bool:
        """Verify randomness signature."""
        return bls.Verify(cast(BLSPubkey, pubkey), message, cast(BLSSignature, signature))

    def verify(self, data: dict, pubkey: str) -> tuple[bool, str | None]:
...
def is_pep604_union(ty: type[Any]) -> bool:
    ...
    return sys.version_info >= (3, 10) and ty is types.UnionType  # type: ignore # pylint: disable=no-member
...
def check_tuple(value: Any, ty: type[tuple[Any, ...]]) -> Result:
    ...
    if len(value) != len(types_):
        return AutonomyTypeError(ty=ty, value=value)
    for v, t in zip(value, types_, strict=False):
        err = check(v, t)
        if is_error(err):
            return err
    return None
...
def check_mono_container(value: Any, ty: type[list[Any]] | type[set[Any]] | type[frozenset[Any]]) -> Result:
...
def is_typeddict(ty: type[Any]) -> TypeGuard[type[TypedDict]]:  # type: ignore
    ...
    #       or, use publich API
    T = "_TypedDictMeta"
    return any(hasattr(mod, T) and isinstance(ty, getattr(mod, T)) for mod in [typing, typing_extensions])
...
def is_json_serializable(obj: Any) -> bool:
    ...
    if isinstance(obj, dict):
        return all(is_primitive_or_none(k) and is_json_serializable(v) for k, v in obj.items())
